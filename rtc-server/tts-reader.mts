...
import { synthesize, sendViaPlainTransport } from './tts-speak.mts';
...
let targetRoomIds: Set<string> | null = null; // null = all

interface TealusMessage {
  type: string;
  sender_id?: string;
...
// --- Tealus API ---
function httpRequest<T = unknown>(url: string, options: http.RequestOptions, body?: string): Promise<HttpResponse<T>> {
  return new Promise((resolve, reject) => {
    const req = http.request(url, options, (res) => {
      ...
      res.on("end", () => {
        ...
        catch { resolve({ status: res.statusCode, data: data as unknown as T }); }
      });
    });
    req.on("error", reject);
...
async function login(): Promise<void> {
  ...
  const res = await httpRequest<LoginResponseData>(`${TEALUS_API}/api/auth/login`, {
    method: "POST",
...
async function getRooms(): Promise<RoomSummary[]> {
  const res = await httpRequest<RoomsResponseData>(`${TEALUS_API}/api/rooms`, {
    method: "GET",
...
// --- テキスト前処理 ---
function preprocessText(content: string | undefined): string | null {
...
function enqueue(roomId: string, text: string, senderName: string): void {
...
async function processQueue(): Promise<void> {
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      const tmpFile = path.join(import.meta.dirname, `.tts-reader-tmp-${Date.now()}.wav`);
...
// --- メッセージフィルタ ---
function shouldRead(msg: TealusMessage): boolean {
  // text 以外はスキップ
...
async function main(): Promise<void> {
  ...
  // 4. メッセージ監視
  socket.on("message:new", (msg: TealusMessage) => {
