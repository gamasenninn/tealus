...
import * as ttsCore from '../agent-server/src/lib/tts-core.mts';
...
// CLI 引数（モジュール利用時は無視）
const ROOM_ID: string | null = import.meta.main ? process.argv[2] : null;
const TEXT: string | null = import.meta.main ? (process.argv[3] || null) : null;
const MODEL_UUID: string = import.meta.main ? (process.argv[4] || DEFAULT_MODEL) : DEFAULT_MODEL;
...
export function sendViaPlainTransport(wavPath: string, roomId: string): Promise<number | null> {
  return ttsCore.sendViaPlainTransport(wavPath, roomId, {
    rtcPort: RTC_PORT,
    ssrc: SSRC,
    onProgress: (time: string) => process.stdout.write(`\r  🔊 ${time}`),
  });
}
