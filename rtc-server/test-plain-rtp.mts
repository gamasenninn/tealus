...
interface PlainTransportCreatedMessage extends WsMessage {
  id: string;
  port: number;
  ip: string;
}

interface ProducedMessage extends WsMessage {
  producerId: string;
}
...
function waitFor<T extends WsMessage = WsMessage>(predicate: (m: WsMessage) => boolean): Promise<T> {
  return new Promise((resolve) => {
    pendingResolvers.push({ predicate, resolve: (m) => resolve(m as T) });
  });
}
...
async function main(): Promise<void> {
  ...
  const ptResp = await waitFor<PlainTransportCreatedMessage>((m) => m.type === "plainTransportCreated");
  ...
  const prodResp = await waitFor<ProducedMessage>((m) => m.type === "produced");
