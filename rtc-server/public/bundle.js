(() => {
  ...
  async function createSendTransport() {
    ...
    sendTransport.on("connect", ({ dtlsParameters }, callback, errback) => {
      ...
      waitForMessage((m) => m.type === "transportConnected" && m.transportId === sendTransport.id).then(() => callback()).catch(errback);
    });
    sendTransport.on("produce", ({ kind, rtpParameters }, callback, errback) => {
      send({ type: "produce", transportId: sendTransport.id, kind, rtpParameters });
      waitForMessage((m) => m.type === "produced").then((m) => callback({ id: m.producerId })).catch(errback);
    });
  }
  async function createRecvTransport() {
    ...
    recvTransport.on("connect", ({ dtlsParameters }, callback, errback) => {
      ...
      waitForMessage((m) => m.type === "transportConnected" && m.transportId === recvTransport.id).then(() => callback()).catch(errback);
    });
  }
