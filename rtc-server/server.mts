...
function dnsLookup(): Promise<string> {
  return new Promise((resolve, reject) => {
...
async function detectPublicIp(): Promise<string | null> {
  ...
  try {
    ...
    logger.warn("DNS lookup failed:", e instanceof Error ? e.message : String(e));
  }
  ...
  for (const url of apis) {
    try {
      ...
      logger.warn(`${url} failed:`, e instanceof Error ? e.message : String(e));
    }
  }
...
function getListenInfos(publicIp: string | null): mediasoup.types.TransportListenInfo[] {
...
interface RtcServerConfig {
  listenPort: number;
  mediasoup: {
    worker: mediasoup.types.WorkerSettings;
    router: { mediaCodecs: mediasoup.types.RouterRtpCodecCapability[] };
    webRtcTransport: {
      listenInfos: mediasoup.types.TransportListenInfo[];
      enableUdp: boolean;
      enableTcp: boolean;
      preferUdp: boolean;
    };
...
const config: RtcServerConfig = {
  listenPort: Number(process.env.RTC_PORT) || 3100,
  mediasoup: {
    worker: {
      rtcMinPort: 10000,
      rtcMaxPort: 10100,
...
interface Peer {
  ws: WebSocket | null;
  transports: Record<string, mediasoup.types.WebRtcTransport | mediasoup.types.PlainTransport>;
...
async function createWebRtcTransport(): Promise<mediasoup.types.WebRtcTransport> {
  const transport = await router.createWebRtcTransport(config.mediasoup.webRtcTransport as mediasoup.types.WebRtcTransportOptions);
...
function createPeer(roomId: string, peerId: string, ws: WebSocket): void {
...
function getRoomPeers(roomId: string | null): Room {
  if (roomId === null) return new Map();
  return rooms.get(roomId) || new Map();
}

// --- JWT authentication for WebSocket ---
interface WsAuthUser {
  id: string;
}

function authenticateWs(req: http.IncomingMessage): WsAuthUser | null {
  ...
  try {
    return jwt.verify(token as string, JWT_SECRET) as WsAuthUser;
  } catch (e) {
    logger.warn("JWT verification failed:", e instanceof Error ? e.message : String(e));
    return null;
  }
}

// --- Keepalive-augmented WebSocket ---
interface KeepaliveSocket extends WebSocket {
  isAlive?: boolean;
}

// --- クライアント → サーバー シグナリングメッセージ ---
...
// --- WebSocket signaling ---
function handleWebSocket(ws: WebSocket, req: http.IncomingMessage): void {
  // JWT 認証（token がなければ匿名モード — テスト用）
  const user = authenticateWs(req);
  let peerId: string | null = user?.id || null;
  let roomId: string | null = null;

  // keepalive
  const aliveWs = ws as KeepaliveSocket;
  aliveWs.isAlive = true;
  aliveWs.on("pong", () => { aliveWs.isAlive = true; });
    ...
    try {
      switch (msg.type) {
        ...
        case "connectTransport": {
          const room = getRoomPeers(roomId);
          const peer = room.get(peerId!)!;
          const transport = peer.transports[msg.transportId!] as mediasoup.types.WebRtcTransport;
        ...
        case "getProducers": {
          const room = getRoomPeers(roomId);
          const producers: Array<{ producerId: string; producerPeerId: string; kind: mediasoup.types.MediaKind }> = [];
          for (const [otherId, otherPeer] of room) {
            if (otherId === peerId) continue;
...
async function main(): Promise<void> {
  ...
  const bundlePath = path.join(import.meta.dirname, "public", "bundle.js");
  ...
  setInterval(() => {
    wss.clients.forEach((ws) => {
      const client = ws as KeepaliveSocket;
