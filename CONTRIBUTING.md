...
# 特定のテストのみ
cd server && npm test -- --testPathPattern="tags"
```
...
- **小さな修正（typo / 1 ファイル内の small fix）**: main への直接 commit でも可（メンテナのみ）
- **機能追加・大規模変更・破壊的変更**: フィーチャーブランチを切って **PR** を出す
- **外部コントリビュータ**: 必ず fork → PR ベース
- main は branch protection 設定済み（linear history、force push 禁止）
...
- [ ] 破壊的変更がある場合、PR 本文で明記
- [ ] OSS 訪問者に見える文書（README / docs.tealus.dev）に default 値・実在ドメイン・弱パスワードを残していないか確認
...
- 一般的な議論: 現状は Issue で受け付け（Discussions は要望があれば有効化）
