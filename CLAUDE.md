...
- `docs/08_会話モード設計.md` — 組織記憶に入れないための入口 (#389)。**目的は「声で話せること」ではない**ので §1.2 を先に読むこと (昇格が無いなら作らない、が成立条件)。**「音声を速くする」話を始める前に §2 の実測と §6.0 を読むこと** (律速は音声でなく毎ターンの文脈量。速さを後回しにできるのは主用途を壁打ちに置いたからで、雑談に戻すと判断が崩れる)。★ **2026-09-05 に §12 (Realtime API 経路) を増築した。条件つきで、§7.1 の基準 4 項目を満たさなければ §12 ごと捨てて §11 までの姿に戻る** — 冒頭の「★ 転回」を先に読むこと。§1〜§11 は既存 agent 経路の設計として全部生きている (**2 つの経路が併記されているので、どちらの話かを必ず確かめる**)
