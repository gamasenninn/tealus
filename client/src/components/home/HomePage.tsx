...
// お知らせ API は is_unread を、media は file_path を返す (types.ts には未定義)
type AnnouncementMessage = Omit<Message, 'media'> & {
  is_unread?: boolean;
  media?: Array<MediaItem & { file_path?: string }>;
};
...
function HomePage() {
  ...
  const [portalLinks, setPortalLinks] = useState<Array<{ id: string; title: string; url: string }>>([]);
  ...
  const loadAnnouncements = async () => {
    try {
      ...
      setAnnouncements((data.messages || []) as AnnouncementMessage[]);

      // 未読メッセージを既読にする
      const unreadMsgs = ((data.messages || []) as AnnouncementMessage[]).filter(m => m.is_unread);
  ...
  const parseTitle = (content: string): { title: string | null; body: string } => {
