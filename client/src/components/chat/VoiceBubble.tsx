...
// 無いため local に widen する (socket 'voice:status' が届ける値)。
type VoiceTranscription = Omit<Transcription, 'status'> & {
  status: Transcription['status'] | 'transcribing' | 'formatting';
};
...
interface VoiceBubbleProps {
  ...
  isOwn: boolean;
  canEditTranscription?: boolean;
  replyMessage?: QuotedMessage | null;
  searchKeyword?: string | null;
}

function VoiceBubble({ message, media, transcription, isOwn, canEditTranscription, replyMessage, searchKeyword }: VoiceBubbleProps) {
  ...
  useEffect(() => {
    const editHandler = (e: Event) => {
      if ((e as CustomEvent<{ messageId: string }>).detail.messageId === message.id) handleStartEdit();
    };
    const historyHandler = (e: Event) => {
      if ((e as CustomEvent<{ messageId: string }>).detail.messageId === message.id) handleShowHistory();
    };
    const playHandler = (e: Event) => {
      if ((e as CustomEvent<{ messageId: string }>).detail.messageId === message.id) {
  ...
  useEffect(() => {
    const handleOtherPlay = (e: Event) => {
      if ((e as CustomEvent<{ messageId: string }>).detail.messageId !== message.id && isPlaying) {
