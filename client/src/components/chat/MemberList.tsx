...
// server の room members 応答は user_id を持ち、role は room 内 role ('admin' | 'member')。
// types.ts の RoomMember (role = UserRole) と食い違うため local に補正する。
type MemberEntry = Omit<RoomMember, 'role'> & { user_id: string; role?: string };
...
function MemberList({ roomId, onClose }: MemberListProps) {
  const { user } = useAuthStore();
  const { currentRoom, members, selectRoom } = useRoomStore();
  const memberList = members as MemberEntry[];
