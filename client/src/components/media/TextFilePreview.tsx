...
/** 判定・preview に必要な media (= message_media row) の最小形 */
 ...
 */
export function isTextFile(media: PreviewMedia | null | undefined): boolean {
 ...
 */
export function isMarkdownFile(media: PreviewMedia | null | undefined): boolean {
  if (!media) return false;
  const mime = (media.mime_type || '').toLowerCase();
 ...
 */
export function isJsonFile(media: PreviewMedia | null | undefined): boolean {
  if (!media) return false;
  const mime = (media.mime_type || '').toLowerCase();
 ...
 */
export function isCsvFile(media: PreviewMedia | null | undefined): boolean {
  if (!media) return false;
  const mime = (media.mime_type || '').toLowerCase();
 ...
 */
export function formatJson(text: string): { formatted: string; ok: boolean } {
  try {
 ...
 */
export default function TextFilePreview({ media }: TextFilePreviewProps) {
  ...
  const renderBody = () => {
    // renderBody は content !== null のガード内でのみ呼ばれる
    ...
    if (isJson) {
      const { formatted, ok } = formatJson(text);
      return (
        <>
    ...
    if (isCsv) {
      const rows = parseCsv(text, csvSeparator);
      if (rows.length === 0) return <pre className="text-file-preview__pre">{text}</pre>;
    ...
    return <pre className="text-file-preview__pre">{text}</pre>;
  };
