...
/** socket 'stamp:generated' payload (生成完了通知) */
interface StampGenerateResult {
  pack: { name: string };
  count: number;
  stamps: Array<{ filePath: string; label?: string | null }>;
}
...
function StampGenerator({ onClose }: StampGeneratorProps) {
  ...
  const [result, setResult] = useState<StampGenerateResult | null>(null);
  ...
  useEffect(() => {
    ...
    const handleGenerated = (data: StampGenerateResult) => {
      setGenerating(false);
      setResult(data);
    };
  ...
  const handleGenerate = async () => {
    ...
    setError('');
    try {
      await api.generateStampPack(prompt.trim(), (name.trim() || undefined) as string, roomId, activeLabels);
    } catch (err) {
      setGenerating(false);
        ...
        <div className="stamp-generator-result">
          ...
          <div className="stamp-generator-preview">
            {result.stamps.map((s, i) => (
              <img key={i} src={`/media/${s.filePath}`} alt={s.label ?? undefined} />
            ))}
          </div>
