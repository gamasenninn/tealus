...
// server 応答は is_active を含む (types.ts の PortalLink には未定義)
type PortalLinkRow = PortalLink & { is_active?: boolean };
...
function PortalManager() {
  ...
  const loadLinks = async () => {
    try {
      ...
      setLinks(data.links as PortalLinkRow[]);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const data: Partial<PortalLink> = { title: formTitle, url: formUrl, icon: formIcon || null };
      if (editingId) {
  ...
  const handleEdit = (link: PortalLinkRow) => {
  ...
  const handleDelete = async (link: PortalLinkRow) => {
