...
const catLabel = (c: string | null | undefined) => (CATEGORIES.find((x) => x.key === c) || ({} as { label?: string })).label || c;

// server の alias 応答行 (types.ts の DictionaryAlias とはフィールド形が異なる):
// alias_id / term / alias / source / status / count / reading を返す
interface AliasRow {
  alias_id: string;
  term: string;
  alias: string;
...
// server の term 応答行 (types.ts の DictionaryTerm + description / alias_count)
type TermRow = DictionaryTerm & { description?: string | null; alias_count?: number };
...
function AliasView() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setAliases((data.aliases || []) as unknown as AliasRow[]);
    } catch (err) { setError(err instanceof Error ? err.message : String(err)); } finally { setLoading(false); }
  };
  const approve = async (id: string) => {
    try { await api.approveDictionaryAlias(id); await load(); } catch (err) { setError(err instanceof Error ? err.message : String(err)); }
  };
  const reject = async (id: string) => {
    try { await api.rejectDictionaryAlias(id); await load(); } catch (err) { setError(err instanceof Error ? err.message : String(err)); }
  };
...
function TermView() {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [editing, setEditing] = useState<EditingTerm | null>(null); // { id, reading, description, category }
