...
import type { Room, Webhook } from '../../types';
...
// server 応答は room 名を JOIN 済みで返す (types.ts の Webhook には未定義)
type WebhookRow = Webhook & { room_name?: string | null };
...
function WebhookManager() {
  ...
  const handleEdit = (webhook: WebhookRow) => {
  ...
  const handleTest = async (webhook: WebhookRow) => {
    ...
    try {
      const result = await api.testWebhook(webhook.id) as { success?: boolean; status?: number };
