...
実 demo では実 user (小野哲)、実 organization の語彙 (ガマ、みこがい、田植機、ビレッジ側等) を使う。**公開資料では anonymize**:
...
**実機 demo は agriculture context、公開資料は generic な「業務組織」context** で書き分けると audience の幅が広がる。

→ Anonymize 作業は別 issue 候補。
