...
- **OpenAI API**: Whisper (転写) + gpt-4o-mini (整形 legacy / Light v1 / Router LLM) + gpt-5.4-mini (Light v1 model)
  - 整形 organon モードのモデルは `ORGANON_AI_MODEL` (既定 gpt-5.4-mini)。★ **補正の指示はモデルの系統ごと** (luna 系は別の指示、組織固有名詞リストは共通)。
    モデルを替えれば指示も替わる。系統を足すときは同じ試験で測ってから (`docs/05` の「整形段のモデルを替えるとき」)
...
- **OpenAI Images API** (`gpt-image-1`、env `OPENAI_IMAGE_MODEL` で上書き可。generate_and_send_image MCP tool、opt-in。旧 DALL-E 3 は #313 で移行)
...
> **#330 (2026-07) 以降のファイル拡張子:** 下図は Phase 1 baseline の例示。実際はサーバ / agent-server / rtc-server の実行コードは `.mts`（Node native type stripping でビルドレス実行）、クライアント / dashboard は `.ts` / `.tsx`（Vite）。`.js` / `.jsx` はプロダクトコードには残っていない（設定ファイル等の一部を除く）。
...
- env `ORGANON_INJECT=true` で全 agent 共通 inject を opt-in、未設定 / それ以外は無効 (default OFF、#304)。organon は本体と分離可能なサブシステムのため使う deployment だけが明示有効化。旧 `INJECT_ORGANON_POLYSEME` (default ON の opt-out) は廃止・fallback 無し。起動ログ「organon inject: ON/OFF」で状態確認可
