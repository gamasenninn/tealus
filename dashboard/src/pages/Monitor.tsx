...
import type { AgentStatsResponse, AgentLogsResponse, AgentLogContextResponse } from '../services/api';
import type { ServerLogsResponse } from '../services/agentApi';
...
function Monitor() {
  ...
  const [logs, setLogs] = useState<AgentLogsResponse>({ messages: [], total: 0 });
  const [logPage, setLogPage] = useState(0);
  const [logDetail, setLogDetail] = useState<AgentLogContextResponse | null>(null);
  const [logRoomFilter, setLogRoomFilter] = useState('');
  const [serverLogs, setServerLogs] = useState<ServerLogsResponse>({ logs: [], total: 0, date: '' });
  const [srvLogDate, setSrvLogDate] = useState('');
  const [srvLogDates, setSrvLogDates] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    const handleAgentStatus = (data: AgentStatusEventPayload) => {
      setEvents(prev => [{
    ...
    const handleMessage = (data: MessageNewEventPayload) => {
      setEvents(prev => [{
        time: new Date().toLocaleTimeString('ja-JP'),
        type: 'message:new',
  ...
  // 応答ログ詳細
  const handleLogClick = async (messageId: string) => {
    try {
