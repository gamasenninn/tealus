...
import { dispatchWebhook } from '../../services/webhook.mts';

export const router = express.Router();

/** GET /webhooks の一覧行 (webhooks + rooms join) */
interface WebhookListRow {
  ...
  is_active: boolean;
  created_at: Date;
  dm_member_names: string | null;
}

/** POST /webhooks/:id/test で使う webhooks 行 (SELECT *。送信に使うカラムのみ型付け) */
...
router.get('/webhooks', async (req, res) => {
  try {
    const result = await pool.query<WebhookListRow>(
...
router.post('/webhooks/:id/test', async (req, res) => {
  ...
  try {
    const result = await pool.query<WebhookRow>('SELECT * FROM webhooks WHERE id = $1', [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Webhookが見つかりません' });
    }
    ...
    res.status(502).json({ error: 'テスト送信に失敗しました', details: err instanceof Error ? err.message : String(err) });
  }
});
