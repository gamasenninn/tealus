...
/** GET /agent-logs のカウント行 */
interface CountRow {
  total: number;
}

/** GET /agent-stats の応答回数集計行 */
interface StatsRow {
...
router.get('/agent-stats', async (req, res) => {
  try {
    // Bot ユーザーの ID 取得
    const bots = await pool.query<{ id: string }>('SELECT id FROM users WHERE is_bot = true');
    ...
    const avgResult = await pool.query<AvgRow>(
...
router.get('/agent-logs', async (req, res) => {
  ...
  try {
    const botIds = (await pool.query<{ id: string }>('SELECT id FROM users WHERE is_bot = true')).rows.map(b => b.id);
    if (botIds.length === 0) return res.json({ messages: [], total: 0 });

    const conditions = ['m.sender_id = ANY($1)', 'm.is_deleted = false'];
    const params: Array<string | string[] | number> = [botIds];
    let paramIdx = 2;

    if (room_id) {
      conditions.push(`m.room_id = $${paramIdx++}`);
      params.push(String(room_id));
    }
...
/** Bot 応答メッセージ行 (messages.* + room_name。ハンドラで使うカラムのみ型付け) */
...
/** 直前のユーザーメッセージ行 (messages.* + sender_display_name。ハンドラで使うカラムのみ型付け) */
...
router.get('/agent-logs/:id/context', async (req, res) => {
  ...
  try {
    // Bot の応答メッセージを取得
    const botMsg = await pool.query<BotMessageRow>(
    ...
    const userMsg = await pool.query<QuestionRow>(
    ...
    if (question && question.type === 'voice') {
      const trans = await pool.query<TranscriptionRow>(
