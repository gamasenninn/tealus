...
/** messages テーブルの行 (SELECT * / RETURNING *) */
...
/** message_media テーブルの行 (RETURNING *)。BIGINT (file_size) は pg が string で返す */
interface MediaRow {
  id: string;
  message_id: string;
  file_path: string;
...
/** GET /messages の行 (m.* + sender_display_name)。reactions / transcription は後段で付与 */
interface BotMessageListRow extends MessageRow {
...
/** voice_transcriptions の verbosity 制御 SELECT 行 (#219: 列は動的に増減) */
interface TranscriptionVerbosityRow {
  id: string;
  status: string;
  version: number;
...
/** voice_transcriptions の text 3 列 (media / cache 返却用) */
interface TranscriptionTextRow {
  raw_text: string | null;
  formatted_text: string | null;
  status: string;
}
...
const ttsAudioCache = new Map<string, { buffer: Buffer; contentType: string; expiresAt: number }>(); // id → { buffer, contentType, expiresAt }
...
router.post('/push', async (req, res) => {
  const { room_id, content, type = 'text' } = req.body as { room_id?: string; content?: string; type?: string };
...
router.post('/tts-speak', async (req, res) => {
  const { room_id, text } = req.body as { room_id?: string; text?: string };
  const userId = req.user!.id;
...
router.post('/status', async (req, res) => {
  const { room_id, status, message } = req.body as { room_id?: string; status?: string; message?: string };
  const userId = req.user!.id;
...
router.get('/messages', async (req, res) => {
  ...
  try {
    ...
    if (messages.length > 0) {
      ...
      const rx = await pool.query<ReactionAggRow>(
      ...
      const byMsg = new Map<string, { emoji: string; count: number }[]>();
    ...
    for (const msg of messages) {
      if (msg.type === 'voice') {
        const trans = await pool.query<TranscriptionVerbosityRow>(
...
/** GET /messages/:id/media の response 形 (error / data_base64 / transcription は条件付き) */
interface BotMediaResponse {
  message_id: string;
  type: string;
  media_count: number;
  media: { index: number; file_name: string; mime_type: string; file_size: number }[];
  index: number;
  mime_type: string;
  file_name: string;
  file_size: number;
...
router.get('/messages/:id/media', async (req, res) => {
  ...
  try {
    // 1. メッセージ meta
    const msgRes = await pool.query<Pick<MessageRow, 'id' | 'type' | 'room_id' | 'is_deleted'>>(
    ...
    const mediaRes = await pool.query<Pick<MediaRow, 'file_path' | 'file_name' | 'mime_type' | 'file_size'>>(
    ...
    const parsed = parseInt(req.query.index as string, 10);
    ...
    const response: BotMediaResponse = {
      message_id: msg.id,
      type: msg.type,
    ...
    if (msg.type === 'voice') {
      const trans = await pool.query<TranscriptionTextRow>(
...
router.post('/messages/:id/transcribe', async (req, res) => {
  ...
  try {
    ...
    if (!forceRetranscribe) {
      const cached = await pool.query<TranscriptionTextRow & { version: number }>(
    ...
    const finalResult = await pool.query<TranscriptionTextRow>(
      `SELECT raw_text, formatted_text, status
       FROM voice_transcriptions
       WHERE message_id = $1 AND version = $2`,
      [messageId, newVersion]
    );
    const final: Partial<TranscriptionTextRow> = finalResult.rows[0] || {};
...
router.get('/messages/:id/edit-history', async (req, res) => {
  ...
  try {
    // メッセージ + ルーム所属を一括チェック
    const msgResult = await pool.query<Pick<MessageRow, 'id' | 'type' | 'room_id' | 'sender_id' | 'content' | 'is_edited' | 'is_deleted' | 'created_at'>>(`
...
function buildSnippet(text: string | null, query: string | undefined, contextChars: number = 100): string {
...
/** GET /search の結果行 (Case 1 / Case 2 両 path 共通の SELECT 列) */
interface SearchResultRow {
  ...
  room_type: string;
  transcription_formatted: string | null;
...
router.get('/search', async (req, res) => {
  const {
    ...
    q?: string; room_id?: string; sender_id?: string; type?: string;
    tag_names?: string; is_done?: string | boolean; since?: string; until?: string;
    has_reaction?: string | boolean; limit?: string | number; offset?: string | number;
  };
  const userId = req.user!.id;
  ...
  try {
    ...
    let paramIdx = 1;
    const filtersSQL: string[] = []; // 共通の追加 WHERE (Case 1 / 2 path で使い回し)
    ...
    const result = await pool.query<SearchResultRow>(query, params);
...
router.patch('/messages/:id/tags/:tag_name/done', async (req, res) => {
  ...
  try {
    // メッセージの所属ルーム取得 + 所属検証
    const msgRes = await pool.query<{ room_id: string }>(
...
/** GET /unread の行 (messages + sender/room JOIN 列)。voice の content は後段で transcription に差し替え */
interface UnreadRow {
  id: string;
  room_id: string;
  sender_id: string;
...
router.get('/unread', async (req, res) => {
  ...
  try {
    ...
    for (const msg of result.rows) {
      if (msg.type === 'voice') {
        const trans = await pool.query<{ formatted_text: string | null; raw_text: string | null }>(
          `SELECT formatted_text, raw_text FROM voice_transcriptions
...
router.post('/mark-read', async (req, res) => {
  const { message_ids } = req.body as { message_ids?: unknown };
  const userId = req.user!.id;
...
/** GET /rooms の行 (rooms + member_count + direct 相手の LATERAL 列) */
interface BotRoomRow {
  id: string;
  type: string;
  name: string | null;
...
router.get('/rooms', async (req, res) => {
  ...
  try {
    const result = await pool.query<BotRoomRow>(
