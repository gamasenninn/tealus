...
/** stamp_packs 行 (= migration 008) */
interface StampPackRow {
  id: string;
  name: string;
  prompt: string | null;
...
/** messages 行 (= system message insert の RETURNING *、broadcast 用) */
...
router.post('/generate', async (req, res) => {
  ...
  (async () => {
    try {
      ...
      // Create pack in DB
      const packRes = await pool.query<StampPackRow>(
      ...
      try {
        io.to(`user:${userId}`).emit('stamp:error', { jobId, error: err instanceof Error ? err.message : String(err) });
      } catch {
...
router.get('/packs', async (req, res) => {
  const userId = req.user!.id;
  try {
    const result = await pool.query<StampPackRow & { creator_name: string; stamp_count: number; my_last_used_at: Date | null }>(
...
router.get('/packs/:id', async (req, res) => {
  ...
  try {
    const packRes = await pool.query<StampPackRow & { creator_name: string }>(
      `SELECT sp.*, u.display_name AS creator_name
       FROM stamp_packs sp
       JOIN users u ON u.id = sp.created_by
       WHERE sp.id = $1`,
    ...
    const stampsRes = await pool.query<StampRow>(
...
router.delete('/packs/:id', async (req, res) => {
  ...
  try {
    const pack = await pool.query<{ created_by: string | null }>(
    ...
    const packName = (await pool.query<{ name: string }>('SELECT name FROM stamp_packs WHERE id = $1', [packId])).rows[0]?.name || '';
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    const stamp = await pool.query<{ id: string; created_by: string | null }>(
