...
export interface ProxyCloseFacts {
   ...
   * probe-b   ★ UA を明示しない → ★★ ua=curl/8.7.1 (curl の既定)
   * 本線      ★ UA を明示しない → ★★ ua=curl/8.7.1   ← ★★★ probe-b と区別が付かない
   * ```
   * ★★★★ 2026-09-10 訂正: 上を当初「UA を送らない → `ua=-`」と書いていたが、**`ua=-` は
   *   一度も出ていない**。残っている全ログ (438 行) の内訳は `curl/8.7.1` 291 / `cc-probe-c` 147 で、
   *   ★ **curl は既定の UA を送る**。★★ 結論 (= probe-b と本線が分けられない) は変わらないが、
   *   **`ua=-` を探すと永久に見つからない**ので直した。
   * → **`ua=curl/8.7.1` は 1 本を指さない。** 本線に `-A cc-main` が入って初めて 3 本に分かれる。
   *   最初の申告は「probe が UA を送る」だったが、それは probe-c だけの話だった。
   *   **この module は送り手を知らない。`ua=curl/8.7.1` を「本線」と読む規則をここに書かないこと。**
   */
  ua?: string;
}
