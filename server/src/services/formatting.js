...
- **絶対禁止**: 空文字、「空文字」「空文字列」「(空)」「内容なし」「無音」「empty」「null」「none」等のメタ表現を返してはならない。
...
async function formatTranscription(messageId, rawText, io, roomId, version = null, messageType = 'voice') {
  ...
  try {
    ...
    // system prompt で禁止指示しているが、model variance 対策として post-process でも catch。
    if (isMetaEmptyLiteral(formattedText)) {
      logger.warn(`[format] AI returned meta-empty literal "${formattedText}" → fallback to raw_text for message ${messageId}`);
      formattedText = rawText;
    }
    // 完全空文字も raw_text に fallback (raw_text に内容がある場合のみ)
    if (!formattedText && rawText) {
      logger.warn(`[format] AI returned empty string but raw_text has content → fallback to raw_text for message ${messageId}`);
      formattedText = rawText;
    }
