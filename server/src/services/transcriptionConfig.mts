 ...
 *
 * ★ 棄権ガード (#371、2026-08-13)。「リストに無い固有名詞を作らない」は **リスト外** への捏造しか
 * 禁じておらず、実測の誤りは **リスト内** の名前への無理な吸着だった —— 「サマさん」(正: ガマさん) が
 * 3 回で 三和さん / 山崎さん / 高山さん になる。崩れたまま残れば人が気づけるが、実在の同僚名に化けると
 * 気づけない (無線は「誰が誰に頼んだか」の記録なので、静かに嘘になる)。
 * 342 本 × 3 回: 人名の捏造 24.0 → 15.0 (範囲が重ならない)。得 37.0 → 34.0 は幅 8 の中で差とは言えない。
 * ablation: 音韻近接 (L1) だけでは人名の捏造が場所・製品へ移るだけで総数が減らないため、人名を名指しする
 * 2 行 (L2/L3) まで入れて初めて総数が下がる。
 *
 * @param config - loadGuideline() の戻り (vocabulary を参照)
...
- リストの語に直してよいのは、生テキストの読みがその語(または転写ブレ例)の読みと十分に近いときだけ。近い語が無ければリストの語を当てはめず、聞こえたままの表記で残す(カタカナのままでよい)。
- 特に人名は「リストに載っているから」という理由だけで当てはめない。音が合わないなら崩れた表記のまま残すこと。誤った人物名を書くことは、表記が崩れたまま残すことより悪い。
- 話者が名前を呼んでいない箇所に、人名を補わない。
