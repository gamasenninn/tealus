#!/usr/bin/env node
/**
 * mine_transcription_aliases.mts — voice_transcriptions の編集履歴から alias 候補を mining する
 *
 ...
 *
 * 使い方:
 *   node scripts/mine_transcription_aliases.mts [options]
 *
 * Node 20+ の --env-file で .env を読み込む例:
 *   node --env-file=.env scripts/mine_transcription_aliases.mts --threshold=2
 *
 * Options:
...
} from '../src/services/aliasMiner.mts';
import type {
  TranscriptionVersionRow,
  ExtractedAlias,
  AggregatedAlias,
  ExistingVocabularyEntry,
  NewTermCandidate,
  AliasAdditionCandidate,
} from '../src/services/aliasMiner.mts';
...
interface MergeCandidates {
  newTerms: NewTermCandidate[];
  aliasAdditions: AliasAdditionCandidate[];
}
...
export function parseArgs(argv: string[]): ParsedArgs {
  const args: ParsedArgs = {
    ...
    since: null,
    limit: null,
    guidelinePath: path.join(import.meta.dirname, '../config/transcription_guideline.json'),
    reportPath: path.join(import.meta.dirname, '../config/mining_report.json'),
    mode: 'pair',
  ...
  for (const arg of argv.slice(2)) {
    ...
    switch (key) {
      ...
      case 'limit':
        ...
        break;
      case 'guideline-path':
        args.guidelinePath = value as string;
        break;
      case 'report-path':
        args.reportPath = value as string;
        break;
      case 'mode':
...
function printHelp(): void {
  console.log(`mine_transcription_aliases.mts — voice_transcriptions 編集履歴から alias 候補を mining
...
export function loadExistingVocabulary(guidelinePath: string): ExistingVocabularyEntry[] {
  if (!fs.existsSync(guidelinePath)) return [];
  try {
    const raw = fs.readFileSync(guidelinePath, 'utf8');
    const parsed: { vocabulary?: ExistingVocabularyEntry[] } = JSON.parse(raw);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const result = await pool.query<TranscriptionVersionRow>(query, params);
    console.log(`[mine] Fetched ${result.rows.length} transcription rows`);

    const pairs = buildPairsFromRows(result.rows);
    ...
    for (let i = 0; i < targetPairs.length; i++) {
      ...
      const aliases = await extractAliases(pair, openai, args.model ? { model: args.model } : {});
      if (aliases.length > 0) {
    ...
    const aggregated = aggregateAliases(allAliases, {
      mode: args.mode,
    ...
    const belowThreshold = aggregateAliases(allAliases, 1).filter(a => a.count < args.threshold);

    const existingVocab = loadExistingVocabulary(args.guidelinePath);
    const candidates = buildMergeCandidates(aggregated, existingVocab);
...
function writeReport(
  ...
  aliasesRaw: number,
  aliasesAboveThreshold: number,
  candidates: MergeCandidates,
  belowThreshold: AggregatedAlias[],
): void {
  const report = {
