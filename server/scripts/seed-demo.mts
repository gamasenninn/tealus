...
if (BANNED_DB_NAMES.includes(DB_NAME)) {
  ...
  console.error('[seed-demo]      cd server && DB_NAME=tealus_demo node scripts/seed-demo.mts --confirm');
  process.exit(1);
}
...
if (!CONFIRM) {
  ...
  console.error(`[seed-demo]   DB_NAME=${DB_NAME} node scripts/seed-demo.mts --confirm`);
  process.exit(1);
}
...
async function seedUsers(client: pg.PoolClient): Promise<UserMap> {
  const userMap: UserMap = {};
  const hash = await bcrypt.hash(DEMO_PASSWORD, 10);

  for (const u of USERS) {
    const res = await client.query<UserInsertRow>(
...
async function generateDemoImage(): Promise<DemoImage> {
...
async function seedRoomsAndMessages(client: pg.PoolClient, users: UserMap): Promise<void> {
  // --- DM: alice ↔ assistant ---
  const dm = await client.query<IdRow>(
  ...
  // --- Group: プロジェクトAlpha ---
  const grp = await client.query<IdRow>(
  ...
  // 1. alice: 挨拶
  const m1 = await client.query<IdRow>(
    `INSERT INTO messages (room_id, sender_id, content, type, created_at)
  ...
  // 3. charlie: 画像メッセージ
  const m3 = await client.query<IdRow>(
  ...
  const todoTag = await client.query<IdRow>(
  ...
  const m5 = await client.query<IdRow>(
    `INSERT INTO messages (room_id, sender_id, content, type, created_at)
  ...
  const m6 = await client.query<IdRow>(
    `INSERT INTO messages (room_id, sender_id, content, type, created_at)
  ...
  const m7 = await client.query<IdRow>(
    `INSERT INTO messages (room_id, sender_id, content, type, created_at)
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error('[seed-demo] エラー:', err instanceof Error ? err.message : String(err));
    throw err;
  } finally {
    client.release();
    await pool.end();
