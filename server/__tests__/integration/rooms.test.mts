...
describe('Rooms API', () => {
  let user1: TestUser, user2: TestUser, user3: TestUser;
  ...
  describe('POST /api/rooms', () => {
    ...
    it('should set creator as admin', async () => {
      ...
      const creator = res.body.members.find((m: { user_id: string }) => m.user_id === user1.user.id);
  ...
  describe('DELETE /api/rooms/:id', () => {
    it('should delete a solo bot-only room (creator + only member)', async () => {
      ...
      const ids = listRes.body.rooms.map((r: { id: string }) => r.id);
