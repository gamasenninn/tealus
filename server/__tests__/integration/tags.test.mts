...
describe('Tags API', () => {
  let user1: TestUser, user2: TestUser, roomId: string, messageId: string;
  ...
  describe('GET /api/rooms/:id/tags', () => {
    it('should return tags with usage count', async () => {
      ...
      // 使用中のタグが先に来る
      const usedTag = res.body.tags.find((t: { name: string }) => t.name === '合宿');
    ...
    it('should return empty array for no tags', async () => {
      ...
      const nonTodoTags = res.body.tags.filter((t: { is_todo: boolean }) => !t.is_todo);
  ...
  describe('GET /api/rooms/:id/tags/suggest', () => {
    it('should suggest tags by prefix', async () => {
      ...
      expect(res.body.tags.every((t: { name: string }) => t.name.startsWith('合'))).toBe(true);
    });
  });
