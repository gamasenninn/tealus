...
function iso(v: string | number | Date): number {
  return new Date(v).getTime();
}

describe('GET /api/admin/access-log', () => {
  let admin: TestUser, user1: TestUser, user2: TestUser, roomA: string, roomB: string;
  ...
  beforeEach(async () => {
    ...
    // messages (投稿)
    const insMsg = (room: string, sender: string, ts: string, deleted = false) =>
      pool.query(
    ...
    const insView = (room: string, uid: string, ts: string) =>
      pool.query(
  ...
  it('users: 全ユーザを含み、最終投稿/最終閲覧を集計 (未活動は null)', async () => {
    ...
    const byId = Object.fromEntries(res.body.users.map((u: { id: string }) => [u.id, u]));
  ...
  it('matrix: 投稿/閲覧のある (ユーザ×ルーム) を room 名付きで返す', async () => {
    ...
    const cell = (uid: string, rid: string) =>
      res.body.matrix.find((m: { user_id: string; room_id: string }) => m.user_id === uid && m.room_id === rid);
