...
describe('Bot API', () => {
  let bot: TestUser, user1: TestUser, roomId: string;
  ...
  describe('GET /api/bot/messages — reactions (#324)', () => {
    ...
    it('同一 emoji の複数リアクションは count に集約される', async () => {
      ...
      const msg = res.body.messages.find((m: { id: string }) => m.id === id);
  ...
  describe('GET /api/bot/search — has_reaction (#325)', () => {
    let reactedId: string, plainId: string;
    ...
    it('has_reaction=false はリアクション無しのものだけ返す', async () => {
      ...
      expect(res.status).toBe(200);
      const ids = res.body.results.map((r: { message_id: string }) => r.message_id);
  ...
  describe('GET /api/bot/messages — transcription verbosity (#219)', () => {
    let voiceMsgId: string, voiceMultiVersionMsgId: string, voiceNoTransMsgId: string, textMsgId: string;
    ...
    it('default (flag 未指定): voice transcription は formatted_text のみ inline、raw_text は省略', async () => {
      ...
      expect(res.status).toBe(200);
      const voice = res.body.messages.find((m: { id: string }) => m.id === voiceMsgId);
    ...
    it('text message には verbosity flag に関わらず transcription field が付かない', async () => {
      ...
      expect(res.status).toBe(200);
      const text = res.body.messages.find((m: { id: string }) => m.id === textMsgId);
    ...
    it('voice message に transcription row が無い場合は transcription field を付けない', async () => {
      ...
      expect(res.status).toBe(200);
      const voice = res.body.messages.find((m: { id: string }) => m.id === voiceNoTransMsgId);
    ...
    it('複数 version の transcription は最新版 (version=2) を返す', async () => {
      ...
      expect(res.status).toBe(200);
      const voice = res.body.messages.find((m: { id: string }) => m.id === voiceMultiVersionMsgId);
  ...
  describe('GET /api/bot/messages/:id/media', () => {
    ...
    async function insertImageMessage(senderId: string, rid: string, fileContent = 'PNG-TEST-DATA') {
      const pool = getTestPool();
    ...
    async function insertMultiImageMessage(senderId: string, rid: string, contents: string[]) {
      const pool = getTestPool();
      const msgRes = await pool.query(
        `INSERT INTO messages (room_id, sender_id, type, content) VALUES ($1, $2, 'image', NULL) RETURNING id`,
        [rid, senderId]
  ...
  describe('GET /api/bot/search', () => {
    let user2: TestUser, otherRoomId: string;
    let textMsgId: string, imageMsgId: string, voiceMsgId: string, taggedMsgId: string;
    let oldMsgId: string;
    ...
    it('1. 単純キーワード検索: results に該当メッセージが入る', async () => {
      ...
      expect(res.body.results.some((r: { message_id: string }) => r.message_id === textMsgId)).toBe(true);
    });
    ...
    it('2. voice transcription を q でヒット', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${bot.token}`);
      expect(res.status).toBe(200);
      const found = res.body.results.find((r: { message_id: string }) => r.message_id === voiceMsgId);
    ...
    it('3. room_id で絞ると他ルームのメッセージが含まれない', async () => {
      ...
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${bot.token}`);
      expect(res.status).toBe(200);
      expect(res.body.results.every((r: { room_id: string }) => r.room_id === roomId)).toBe(true);
    });
    ...
    it('4. sender_id で絞ると該当ユーザの発言のみ', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${bot.token}`);
      expect(res.status).toBe(200);
      expect(res.body.results.every((r: { sender_id: string }) => r.sender_id === user1.user.id)).toBe(true);
    });
    ...
    it('5. type=image で画像メッセージのみ', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${bot.token}`);
      expect(res.status).toBe(200);
      expect(res.body.results.every((r: { type: string }) => r.type === 'image')).toBe(true);
      expect(res.body.results.some((r: { message_id: string }) => r.message_id === imageMsgId)).toBe(true);
    });
    ...
    it('6. tag_names + is_done で TODO を絞れる', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${bot.token}`);
      expect(res.status).toBe(200);
      expect(res.body.results.some((r: { message_id: string }) => r.message_id === taggedMsgId)).toBe(true);
    });
    ...
    it('7. since/until で期間外メッセージが含まれない', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${bot.token}`);
      expect(res.status).toBe(200);
      expect(res.body.results.every((r: { message_id: string }) => r.message_id !== oldMsgId)).toBe(true);
    });
    ...
    it('10. 非メンバールームのメッセージは見えない', async () => {
      ...
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${bot.token}`);
      expect(res.status).toBe(200);
      expect(res.body.results.every((r: { room_id: string }) => r.room_id !== otherRoomId)).toBe(true);
    });
    ...
    it('13. snippet にハイライトが含まれる', async () => {
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${bot.token}`);
      expect(res.status).toBe(200);
      const hit = res.body.results.find((r: { message_id: string }) => r.message_id === textMsgId);
    ...
    it('14. snippet 切り詰め (200 文字超は ... が前後に)', async () => {
      ...
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${bot.token}`);
      expect(res.status).toBe(200);
      const hit = res.body.results.find((r: { message_id: string }) => r.message_id === longRes.rows[0].id);
      expect(hit).toBeDefined();
    ...
    it('16. q に LIKE wildcard 文字 (%) を含めても誤動作しない', async () => {
      ...
      expect(res.body.results.every((r: { snippet: string }) => r.snippet.includes('50%'))).toBe(true);
    });
  });
  ...
  describe('GET /api/bot/tags', () => {
    ...
    it('1. bot が auth で全 room の tag 集計を取得', async () => {
      ...
      const tealusTag = res.body.tags.find((t: { name: string }) => t.name === 'tealus関係');
  ...
  describe('PATCH /api/bot/messages/:id/tags/:tag_name/done', () => {
    let messageId: string, tagId: string;
  ...
  describe('GET /api/bot/messages/:id/edit-history', () => {
    let textMsgId: string, voiceMsgId: string;
