 ...
 */
const reading = require('../../src/services/reading');
...
describe('getReadings', () => {
  ...
  test('キャッシュ: 2 回目は pykakasi を呼ばない', async () => {
    let calls = 0;
    const stub = async (ts) => { calls += 1; return Object.fromEntries(ts.map((t) => [t, 'よみ'])); };
  ...
  test('空文字は無視', async () => {
    const stub = async (ts) => Object.fromEntries(ts.map((t) => [t, 'x']));
