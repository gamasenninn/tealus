...
import {
  ...
  type FetchLike,
} from '../../src/services/lineBridge.mts';
...
describe('fetchLineContent', () => {
  test('成功 (= 200) で buffer + mimeType 返却', async () => {
    ...
    const result = await fetchLineContent('msg-id-123', 'access-token-abc', { fetchImpl: mockFetch as unknown as FetchLike });
...
describe('fetchLineStickerImage (= LINE 公式 sticker CDN、Phase 2.2)', () => {
  test('LINE sticker CDN URL から PNG fetch + buffer + mime 返す', async () => {
    ...
    const result = await fetchLineStickerImage('52002734', { fetchImpl: fetchMock as unknown as FetchLike });
  ...
  test('CDN 404 等で throw', async () => {
    ...
    await expect(fetchLineStickerImage('99999', { fetchImpl: fetchMock as unknown as FetchLike })).rejects.toThrow(/404/);
  });
});
...
describe('saveLineContentToFile', () => {
  ...
  test('buffer 以外で throw', async () => {
    await expect(saveLineContentToFile('not buffer' as unknown as Buffer, 'image/png', tmpDir)).rejects.toThrow(/buffer/);
  });
