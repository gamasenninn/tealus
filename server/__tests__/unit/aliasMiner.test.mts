...
import * as aliasMiner from '../../src/services/aliasMiner.mts';

type Pair = Parameters<typeof aliasMiner.extractAliases>[0];
type AggregatedAlias = aliasMiner.AggregatedAlias;
...
describe('extractAliases', () => {
  function makeOpenAI(responseText: string): OpenAI {
    return {
      chat: {
        completions: {
  ...
  test('parses JSON array response', async () => {
    ...
    const result = await aliasMiner.extractAliases(
      { aiText: 'カナさん', userText: 'ガマさん' } as unknown as Pair,
      openai
    );
  ...
  test('strips markdown code fences from response', async () => {
    ...
    const result = await aliasMiner.extractAliases({ aiText: 'a', userText: 'b' } as unknown as Pair, openai);
...
describe('filterSafeAliases (#327 Phase1: 2ゲート + 自動/確認分岐)', () => {
  ...
  test('音韻近 + 高P + freq≥2 → 自動追加 (ホタカ→保坂)', () => {
    const r = aliasMiner.filterSafeAliases(
      [{ from: 'ホタカ', to: '保坂', count: 7 }] as unknown as AggregatedAlias[], opts);
  ...
  test('音韻遠 → 棄却 reason=phonetic (どうも→ガマ)', () => {
    const r = aliasMiner.filterSafeAliases(
      [{ from: 'どうも', to: 'ガマ', count: 9 }] as unknown as AggregatedAlias[], opts);
    expect(r.auto).toEqual([]);
    expect(r.confirm).toEqual([]);
  ...
  test('★低P (常用語) → 棄却 reason=common-word (まさか→保坂)', () => {
    ...
    const r = aliasMiner.filterSafeAliases(
      [{ from: 'まさか', to: '保坂', count: 1 }] as unknown as AggregatedAlias[], opts);
    expect(r.auto).toEqual([]);
    expect(r.confirm).toEqual([]);
