#!/usr/bin/env node
/**
 * Linny CLI — コマンドラインからLinnyにメッセージを送信
 *
 * Usage:
 *   node scripts/linny-cli.js send "Web部" --text "メッセージ"
 *   node scripts/linny-cli.js send @田中太郎 --text "メッセージ"
 *   node scripts/linny-cli.js send "Web部" --image ./screenshot.png
 *   node scripts/linny-cli.js send "Web部" --voice ./recording.mp4
 *   node scripts/linny-cli.js rooms
 */
const fs = require('fs');
...
const SERVER = process.env.LINNY_SERVER || 'http://localhost:3000';
const BOT_ID = process.env.LINNY_BOT_ID;
const BOT_PASS = process.env.LINNY_BOT_PASS;

if (!BOT_ID || !BOT_PASS) {
  console.error('❌ scripts/.env に LINNY_BOT_ID と LINNY_BOT_PASS を設定してください');
  process.exit(1);
}
...
function uploadFile(token, endpoint, fieldName, filePath) {
  return new Promise((resolve, reject) => {
    ...
    const proto = url.protocol === 'https:' ? https : http;
    const boundary = '----LinnyCLI' + Date.now();
...
async function cmdSend(args) {
  ...
  if (!target) {
    ...
    console.error('   例: node scripts/linny-cli.js send "Web部" --text "メッセージ"');
    process.exit(1);
  }
...
switch (command) {
  ...
  case 'rooms':
    ...
    break;
  default:
    console.log('Linny CLI — コマンドラインからLinnyにメッセージ送信');
    console.log('');
    console.log('使い方:');
    console.log('  node scripts/linny-cli.js send "Web部" --text "メッセージ"');
