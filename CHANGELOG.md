...
- **会話モードに手入力を足した** ([#428](https://github.com/gamasenninn/tealus/issues/428)): 画面下部にテキスト入力欄 (Enter でも送れる)。★ **音声の置き換えではなく併存**。
  - ★ **なぜ必要か**: 会話モードは Realtime の音声認識で、**本体の STT 経路 (organon 整形段) を通らない**。整形段は本体側の実測で **+111 語 / +23pt / 壊した語 0 / p<0.0001** ([#426](https://github.com/gamasenninn/tealus/issues/426)) の効き方をする層で、**それが会話モードには丸ごと無い**。★★ しかも音響段・整形段の両方を通しても取れない語がある (`保坂` / `ガマ` / `神山` — 3 つとも辞書にもオントロジーにも入っている)。
  - ★★ **割り込みの規則は音声と同じ** (`response.cancel` + `output_audio_buffer.clear` の 2 段)。経路ごとに変えると「どちらを使っているかで挙動が変わる」ため。
  - ★ 人の入力として音声と同じ欄に残す (`transcript` / `who=user`)。**`source='text'` を添えた** — これが無いと「手入力が誤変換の回避になったか」を後から測れない。
- **会話モードに「もう一度つなぐ」を足した** ([#429](https://github.com/gamasenninn/tealus/issues/429)): 上限やエラーで止まった後、**画面を閉じて開き直すしか戻る道がなかった**。
  - ★ **自動では繋ぎ直さない** —— 「接続しているだけで課金対象」が依頼の動機なので、黙って繋ぐのは逆。**人が押したときだけ**。
  - ★★ 依頼 3 点のうち **2 点は既に済んでいた** — タイムアウトは #414、**MCP を残すは構造的に満たされていた** (MCP は agent-server 側で (agent, room) 単位に 30 分キャッシュされ、音声セッションに紐づかない)。だから**繋ぎ直しは初回ほど待たされない**。
  - ★ 計器も直した: `session_start_request` が初回と繋ぎ直しで同じ印だったため **「やり直した回数」が測れなかった**。`attempt` / `reconnect` を添えた。
- ★ どちらも client 39 suites / **452 tests PASS** (新規 15 本)、`tsc --noEmit` clean、**本番ビルドを配備して実機確認済**。
- **★ 会話モードの門が「走っている」と思い込んだまま戻らない件に、計器を入れた** ([#423](https://github.com/gamasenninn/tealus/issues/423)): `response.created` と終わりの印を `response_lifecycle` として記録に残す。★ **門の挙動は 1 行も変えていない —— まだ直していない。**
  - ★ **直せなかった理由**: 2026-09-05 の実測では最後の音が鳴り終わって **161.5 秒**後に押した発話が断られ、利用者はそのまま閉じた。原因の候補は 3 つ (終わりが来ない / **門が見ていない名前で終わった** / 順序の入れ替え) あるが、**`response.created` と `response.done` をログに残していなかったので測れなかった**。
  - ★★ **`kind` と `active` を並べて残す**のが要点。門は `response.done` **だけ**を見ているので、`kind=finished` なのに `active=true` のままなら「別の名前で終わった」が**その場で確定する**。見たことのない名前は **名前のまま**残す (件数だけでは、次に門へ何を足すか決められない)。
  - ★ 拾う範囲は `response.<名前>` の **2 段だけ**。`response.output_audio_transcript.done` のように **`.done` で終わるのに応答の終わりではない**ものを終わりと読むと、門が早く開いて #405 の二重起動に戻る。
  - ★★★ 集計 (`voice-chat-report`) の表にも出す —— **集計に足しただけでは読まれない** (この repo で踏んだ「受け手が選んだ欄しか出さない」型)。★ **計器より前のログは「0 件」ではなく「記録なし」**と書く: 0 を「問題が無かった」と読ませない。
  - ★ client 39 suites / **460 tests PASS** (新規 8 本)、agent-server 57 suites / **775 tests PASS** (新規 10 本)、両方 `tsc --noEmit` clean、**本番ビルド済**。
...
- **★★ エラー案内が「codex を更新してください」と誤誘導していたのを直した** ([#431](https://github.com/gamasenninn/tealus/issues/431)): `codex_models_manager: failed to refresh available models` を **「CLI が古い」と決め打ち**していた。
  ...
  - ★ **400 本文 (`not supported when using Codex with a ChatGPT account`) は断定的**なので、`model_not_supported` として拾い、モデル設定だけを指す 1 行を出す。★★ 両方が混ざったときは**断定できる方が勝つ**。
  - ★★ **Light v2 が原因別の案内を 1 つも使っていなかった**のも直した。★★★ ただし**分からないときは今までどおり生のエラー抜粋を出す** — 案内文に差し替えると「ログを確認してください」になり、**情報が減る** (2026-09-11 の 400 本文は、そのまま読めば原因が分かる文だった)。
  - ★ 分類名を `cli_outdated` → `models_refresh_failed` に変更。**観測を名前にする** (推測を名前にすると、その推測が案内文に出る)。
...
- **`POST /api/auth/login` に総当たり抑止を入れた** ([#362](https://github.com/gamasenninn/tealus/issues/362)): **15 分に 5 回失敗**で 429 (`Retry-After` つき)。鍵は **(client IP, login_id)**。
  ...
  - ★ **分散攻撃は止まらない** (承知の上)。login_id だけで数えれば止まるが、それは「攻撃者が任意の利用者を締め出せる」ことと同義で、cc-bridge も巻き添えになる。**単一の発信元からの総当たりを止める**に留めた。
  - ★ **応答時間で ID の存在が判別できたのを塞いだ** — 以前は user が見つからないと bcrypt を踏まずに即 401 で、**1〜2ms vs 約 51ms** の差があった。見つからないときもダミー hash と照合する (コストは `SALT_ROUNDS` と揃え、ずれたらテストが落ちる)。
  - ★ **`TRUST_PROXY` を新設** (`.env.example` に記載)。**未設定だと `req.ip` が前段プロキシの IP になり、外から来た全員が同じ鍵に集まる** = 攻撃者が数回失敗するだけで正規利用者を締め出せる。起動時に warn が出る。★★ `X-Forwarded-For` を無条件には信じない —— `/api/auth/login` は **Cloudflare を迂回して origin に直接届く**ため、無条件に信じると IP を詐称できる。
  - カウンタはプロセス内 (Redis を挟まない)。再起動で消えるが**塞ぎすぎない側に倒れる**。鍵は窓ごとに掃除し、上限超過時は**塞いでいる鍵を残して**古いものから落とす。
  - 約束と壊し方は [docs/05](docs/05_実装ノート_不変条件と落とし穴.md) の新節に。server 1087 tests PASS。

### Added

- **STT バックエンドに gemini (音響段語彙つき) を追加** ([#424](https://github.com/gamasenninn/tealus/issues/424)): `STT_BACKEND=gemini` で gemini-3.5-transcribe + 辞書の全 term (`custom_vocabulary`) を音響段に渡す。★ 2026-09-07 実測 (37 便、正解 = 人が編集した最終版): 固有名詞 **現行 50% → 85%**。現行が落とした 17 語のうち 12 語を回収し、現行が取れていた 17 語を 1 つも壊さない。差は全部**音響段への語彙の事前登録**から来ており、補正段では構造的に届かない領域 (崩れが正当な語になると補正段は触れない)。
  - **既定は `openai` のまま** (非破壊)。失敗 (429 / HTTP エラー / タイムアウト / ★ 空文字 / ★ 切断疑い) は openai へ fail-open (local と同じ型)。切断疑いは wav の秒数 × 文字数比で検出し、実測 2 件 (28 秒 → 10 字) をテストに固定。
  - ★ **organon モードでも gemini だけは語彙を渡す** — 「音響段は素通し、知識は補正段へ」の軸の唯一の例外。根拠と約束は docs/05 の新節に。openai / local 経路の挙動は変えていない。
  - ★ **動画の抽出音声は gemini に送らない** (openai 経路のまま) — 実測は 33 秒以下の voice クリップのみで、10〜20 分の朝礼音声は未測定。抽出 mp3 は 1MB 程度で**サイズでは分離できない**ため、`videoAudio` フラグで明示的に分ける。
  - `.env.example` に STT バックエンド 3 択のブロックを新設 (★ `STT_BACKEND` / `QWEN_WORKER_URL` 等、これまで env が 1 つも文書化されていなかった)。
  - ★ 9/3 の実験が「上限で崩壊」と誤読された原因は、**エラー本文を 200 字で切って保存していた**こと — 429 の本文には「どの上限か」(free_tier / limit: 25) が書いてあり、切ると読めない。実験一式と経緯は `report/` (非公開) に保存。

### Fixed

- **ルームの workspace から資格情報を追い出す — 2 つ目の経路** ([#419](https://github.com/gamasenninn/tealus/issues/419)): `<WORKSPACE_ROOT>/<agentId>/<roomId>/` は **filesystem MCP の root** なので、そこに置いたものは agent が `read_file` で読める。`deep.mts` の `.deep_mcp_config.json` は先に直したが、**`deepCodex.mts` が同じ形で `.codex_home/` を作っているのを見ていなかった**。★ **1 つ直して満足し、同じ形をもう一度探さなかった。**
  - 中身は **ChatGPT の OAuth トークン** / **API キーと bot のパスワード** / **codex のセッション記録 (鍵が写り込む)** / ★ **codex の会話履歴・memories・logs の sqlite** (起票時に数えていなかった)。1 ルームあたり約 5,000 ファイル。
  - 置き場を `<WORKSPACE_ROOT>/_codex-homes/<agentId>/<roomId>` へ (`_mcp-configs` と同じ形)。★ **既存分は削除ではなく移動**にした —— 消すと codex のセッション記録ごと消えるが、**読める場所から外すだけでよい**。実機で確認したところ、移した履歴を引き継いで次の Deep が続いている (rollout 149 → 150)。
  - 露出していた期間は **auth.json が 99 日 / config.toml が 84 日**。
- **①が遅いのはルームではなく「その往復で `execute_sql` を呼んだか」** ([#420](https://github.com/gamasenninn/tealus/issues/420)): 「社内DB検索ルームだけ遅い、8.8KB の `light_prompt.md` が重い」という見立ては、310 往復を引き直して**否定された**。`light_prompt.md` が書かれた **29 秒後**のセッションが 8/8 で 2 秒以内、13 分後が 11/11。★ **定数は変数を説明できない** (同じファイルが在る状態で 2 秒以内率は 100% から 0% まで振れる)。最も遅い 12 往復は `light_prompt.md` **無し**の部屋にあった。★ 起点は **n=3 で「ルームが割れている」と書いたこと**。
  - `summarizeVoiceChat` に `latencyByTool` を追加し、レポートに常設した。道具の**実行時間**は前から見えていたが、**「その道具を呼ぶ往復は声が返り始めるのが遅い」**方が見えず、次も「ルームが遅い」から始めてしまう。
- **前の応答が走っている間に話すと、黙って捨てられていた** ([#421](https://github.com/gamasenninn/tealus/issues/421)): 起票時 n=1 だったので 318 往復を引き直したら 4 件あり、**3 つの別々の形**だった (割り込み直後 / ★ 何も鳴っていないのに断られる / 押している最中に喋り始め)。★★ **4 件のうち 2 件は、その直後に利用者が会話を閉じている。**
  - 画面に出すようにした。★ 文言は起票時の案「まだ前の返事が続いています」を**採らない** —— 161 秒 何も走っていないのに断られた形では**嘘になる**。**門の言い分ではなく起きたこと**を出す:「いまの声は送れませんでした。もう一度どうぞ」。
  - `response_create_skipped` に `why` / `pending_tools` / `playing` を載せた。直し方が形ごとに違うのに、**断った理由が記録に無くて分けられなかった**ため。★ 「門が開かないまま」の形はこの変更では治らない (原因未特定)。

## [0.9.0] - 2026-08-09

★ ★ ★ **「予告を、経路の全部の端点から出す」** — v0.8.0 で `__bye` (計画的な停止の予告) を入れたのは **agent-server だけ**だった。ところが別マシンの接続は **本体サーバを通り抜けて** agent-server に届いており([#368](https://github.com/gamasenninn/tealus/issues/368))、**本体サーバには `SIGTERM` / `SIGINT` ハンドラが存在しなかった**ため、本体を再起動するたびに中継が予告なしで消えていた。本体に graceful shutdown を入れ、**購読者を知っている agent-server に一段渡して**予告させた。あわせて **寿命切断の理由もサーバが配る**ようにし([#366](https://github.com/gamasenninn/tealus/issues/366))、クライアントが経過秒から逆算する方式 (時計が動くと誤判定する) を主役から降ろした。別レーンで **LINE の `join` を catalog に通し**([#367](https://github.com/gamasenninn/tealus/issues/367))、**フォーム表示を文字サイズ設定に追随させて横拡張と畳み込みを足した**([#370](https://github.com/gamasenninn/tealus/issues/370))。v0.8.0 以降 10 コミット。

> **★ この版の影響範囲**: `@cc-*` の cc-bridge を使っている環境 (#366 #368) と、LINE ブリッジを新しく足す場面 (#367) に効く。**フォームを使っている場合は全員に画面の変化がある** (#370) ので、**client の再ビルドが要る**。DB migration は追加ゼロ (最新は `026` のまま)。

### ★ 更新時の急所

- **★ client を本番ビルドし直す** — #370 は client のみの変更なので、ビルドしないとフォームの表示は**何も変わらない**。サーバだけ更新しても壊れないが、静かに変化ゼロになる。
- **★ cc-bridge を使っている場合、消費側の skill も差し替える** — `.claude/skills/listen-tealus/SKILL.md` は `main` から取得する運用なので、**タグを打っても配布物は切り替わらない**。差し替えないと、寿命切断の理由 (`reason: "max_age"`) が届いても**古い経過秒の窓で判定し続ける**うえ、ダウン秒数の過大表示と `alive` 行の件数ずれも直らない。
- **本体サーバの停止予告は、agent-server が動いていて初めて届く** — 本体は「自分が落ちる」を知っているが**購読者を知らない**。予告に失敗しても停止処理は warn を出して続行する (agent-server が先に落ちている場合は普通に起こりうる)。

### Added

- **本体サーバの graceful shutdown — 停止を中継に予告する** ([#368](https://github.com/gamasenninn/tealus/issues/368)): `SIGINT` / `SIGTERM` で agent-server の `POST /cc-queue/gateway-bye` を叩き、購読者へ `{"__bye":{"reason":"gateway_restart"}}` を配らせてから閉じる。**本体サーバには `process.on` が `unhandledRejection` / `uncaughtException` の 2 つしか無く、正常系のハンドラが 1 つも無かった。**
  - ★ **1 ホップで済まない初めての形** — 「片側しか持っていない情報は、持っている側から配る」(v0.8.0 で記録) が、ここでは**情報 (自分が落ちる) と配る能力 (購読者を知っている) が別プロセスにある**。どちらか片方では予告できないので、一段渡すしかない。
  - ★ **認可は共有 JWT** — agent-server から見ると、本体の `/agent-api` proxy を通った**外部リクエストも loopback から届く**。「送信元が localhost なら許可」は `https://<host>/agent-api/cc-queue/gateway-bye` で破れるので成立しない。
  - 予告は **1 秒でタイムアウト**する。応答が返らないときに停止処理を人質に取らせないため。トークンも 60 秒の短命。
- **LINE ブリッジ: `join` を catalog に通す** ([#367](https://github.com/gamasenninn/tealus/issues/367)): bot を招待した時点で group ID と名前が catalog (`server/config/line-groups.json`) に載る。従来は `event.type === 'message'` でしか catalog を更新していなかったため、**利用者に「1 通投稿してください」と頼む必要があり**、投稿から mapping を書くまでの間のメッセージは `unmapped-group` として捨てられていた (2026-08-03 の「営業報告」追加時に**画像 1 枚を含む 3 件**が実際に消えている)。LINE ブリッジには cc-bridge の `since` にあたる**取りこぼしを後から拾う仕組みが無い**ので、入口を早める以外に手が無い。★ **catalog upsert だけを通し、投影経路には一切入れない** (`join` は本文を持たないため)。
- **フォームの表示を操作できるようにした** ([#370](https://github.com/gamasenninn/tealus/issues/370)): 業務メモの現場指摘 (項目数の多いフォームが流れを占有する) から。
  - **文字サイズ設定に追随** — `FormBubble.css` がサイズを直書きしていたため、プロフィールの「トーク文字サイズ」を変えても**フォームだけ変わらなかった**。`var(--chat-font-size)` 基準の `calc()` に置換。★ **入力欄だけは `max(16px, ...)`** — iOS Safari は font-size < 16px の input でフォーカス時に自動ズームするので、「小」設定でも 16px を下回らせない (設定への追随より下限が優先される、という例外)。
  - **横拡張** — ★ **仕組みは既にあり、フォームからだけ届いていなかった。** `MessageBubble` のダブルクリックで行幅が 80%→100% になる既存機能を、`FormBubble` が `stopPropagation` (フォーム操作で誤って幅が変わるのを防ぐ、正しい実装) で塞いでいた。新しい幅の仕組みは作らず、タイトル行の操作子から既存の `expanded` を呼ぶだけにした。
  - **畳み込み** — タイトルは残して中身だけ隠す (何のフォームか分かるように)。畳んでも入力値は state に残る。**畳んだ状態は端末に残り、ルーム移動・リロードで開き直さない。** 操作子 2 つはタイトル行に同居させ、**行数は増やしていない**。
  - 見送り: **回答済みフォームの自動折り畳み**。畳むと「回答済みでボタンが締まっている」ことが見えなくなり、二重回答防止 ([#336](https://github.com/gamasenninn/tealus/issues/336) / [#341](https://github.com/gamasenninn/tealus/issues/341)) が効いているか確認できなくなる。

### Changed

- **寿命切断の理由をサーバが伝える** ([#366](https://github.com/gamasenninn/tealus/issues/366)): 55 分の寿命で閉じるときも `{"__bye":{"reason":"max_age"}}` を送る。従来は消費側が**経過秒から理由を逆算**していた (`MAX_AGE <= SEC <= MAX_AGE+5`)。
  - ★ **反証されたのは「`SEC` は原理的に `MAX_AGE` を下回らない」という前提。** 実測で `SEC=3298` が出て、サーバ側の実測は 3300 秒ちょうど (寿命切断で確定)、2 台の時計が 55 分で 2 秒ずれていた (606 ppm = 自然なドリフトでは説明がつかず NTP の補正と見られる)。**導出は正しく、言語化されていない前提が誤っていた** — `date +%s` は**壁時計であって単調増加しない**。この前提は原因が NTP かどうかに関係なく成り立たない。
  - 窓は `MAX_AGE ± 5` の対称にして**古いサーバ向けの退避**として残した。下限を狭くした唯一の根拠が消えたため、非対称にしておく理由がない。
  - `__bye` の中身は **stderr に 1 行残す** — 猶予窓を張るだけだと理由が消える。stdout に出すと起こしてしまうので、**通知はしないが記録は残す**。
- **`alive` 行に集計の経過時間を添える** ([#366](https://github.com/gamasenninn/tealus/issues/366)): `in last 24h` → `in 4h32m`。`DISC` はループのプロセス変数なので**張り直すたびに 0 に戻る**が、ラベルだけは 24h のままだった。「昨日 40 件・今日 6 件、何かあったか」と読める行が、実際は「張り直しただけ」になる (実例あり)。★ `DISC=0` と起点の更新は**必ず同時に**行う。

### Fixed

- **復帰通知のダウン秒数が過大だった** ([#366](https://github.com/gamasenninn/tealus/issues/366)): 起点が**終了した接続の開始時刻**だったため、`down` にその接続が生きていた時間が丸ごと入っていた (実例: 実際のダウン 11 秒に対し `3309s down`)。起点を切断時刻に変更。★ **短命な接続失敗では気づかない** — 長く生きた接続が想定外に終わったとき初めて出る。判定点だけ直して起点を放置していた、同じ勘違いの残り半分。
- **配布前の構文チェックが検査になっていなかった** ([#366](https://github.com/gamasenninn/tealus/issues/366)): v0.8.0 の周知で案内した `sh -n SKILL.md` は、対象が Markdown なので **frontmatter の 3 行目で必ず落ちる**。接続コマンドまで到達せず、**常に赤く出るだけで何も検査していなかった**。接続コマンド部分だけを `awk` で抜き出して `sh -n` に流す形に修正し、**わざと壊した入力で落ちることまで確認**した。

### ドキュメント

- `docs/05_実装ノート_不変条件と落とし穴.md`: **2 ホップ化で増える依存** (本体の停止予告は agent-server が生きていて初めて届く / proxy 経由の外部リクエストは loopback から見える) と、**予告が唯一の判定手段になったこと** (経過秒の窓は退避に降格。ただし古いサーバ向けに残すので消さない) を記録。
- `server/.env.example`: `CC_GATEWAY_EXPECT_BACK_MS` (既定 `30000`) を追加。★ **この値を知っているのは再起動する側 = 本体サーバ**なので、クライアントにハードコードさせない ([#361](https://github.com/gamasenninn/tealus/issues/361) と同じ理由)。

## [0.8.0] - 2026-08-02

★ ★ ★ **「AI セッションを本番から引き剥がす」** — `@cc-{project}` の起床通知の復路が file への追記だけだったため、**AI セッションはサーバと同じホストでしか動かせなかった**。目的は利便性ではなく **隔離** — 本番 DB / メディア / `server/.env` の秘密鍵があるホストと、AI が動くホストを分ける([#214](https://github.com/gamasenninn/tealus/issues/214))。HTTP 経路を file beacon の**横に**生やし(既存経路は不変)、別マシンの Mac セッションで 2 日間の実運用検証を行った。その過程で**認可が接続時のスナップショットのまま更新されない**穴([#360](https://github.com/gamasenninn/tealus/issues/360))を塞ぎ、**定期切断で LLM を起こさない**静音化([#363](https://github.com/gamasenninn/tealus/issues/363))と、**サーバが自分で停止を予告する**仕組み([#365](https://github.com/gamasenninn/tealus/issues/365))を入れて、予告なしの再起動が通知ゼロで通ることを実測した。v0.7.0 以降 22 コミット。

> **★ この版の影響範囲**: 機能の中身は **cc-bridge (`@cc-*` で Claude Code を起こす仕組み) を使っている環境にのみ**効く。メッセンジャーとして使っている採用者に見た目の変化は無い。ただし **Node 要件の宣言が変わった**ので、下の「更新時の急所」は全員が対象。

### ★ 更新時の急所

- **★ Node 24 が必須になった** — `engines` が 6 パッケージとも `>=20` / `>=22.6` から **`>=24.0.0`** に変わった。実行時の要件は以前から 24 (Node native type stripping でビルドレス実行しているため) で、**宣言だけが古かった**のを実態に揃えたもの。Node 22 以下では `npm install` が止まる。
- **★ cc-bridge を使っている場合、消費側の skill も差し替える** — `.claude/skills/listen-tealus/SKILL.md` は `main` から取得する運用なので、**タグを打っても配布物は切り替わらない**。サーバだけ更新すると、新しい制御メッセージを受けても何もしない (壊れはしない — `{"__` の前方互換で黙って無視する)。

### Added

- **cc-bridge の HTTP 化 — AI セッションを別マシンで動かせる** ([#214](https://github.com/gamasenninn/tealus/issues/214)): `GET /cc-queue/stream` が **NDJSON** (1 イベント 1 行) を流し続ける。SSE ではないのは、消費側 (Claude Code の Monitor) から見て `tail -F` と**同じ「行が来る」形**になり、扱いを変えずに済むため。`GET /cc-queue/pending` は **arm する前の疎通確認**用で、200 が返らなければ監視を張らずに止まる (張ると失敗が silent になる)。file beacon への追記は**一切変えていない**。
  - **★ 認可 — 接続者のトークンで参加ルームを引く**: agent-server の `authenticate` は署名検証のみで DB を見ない (疎結合設計) ため、接続時に**呼び出し元のトークンで**本体の `GET /api/rooms` を引き、`allowedRooms` に含まれるルームのイベントだけを配る。beacon を書く bot (agent-server) と消費して返信する bot (CC セッション) は**別 principal で参加ルームが違う**ので、絞らないと「返信できない (bot API が 403) イベントで起こされる」うえ、読み手の権限を超えた本文が HTTP に流れる。実データでも他人の DM が 1 件混ざっていた。この転送は同時に**無効化されたユーザーのトークンの穴も塞ぐ**。
  - **`since` は watermark ではなく受信済みカーソル**: watermark (`.last_processed-*`) は reply 成功時にしか進まないので `since` に使うと**保留中の未処理が再接続のたびに再提示される**。受信した時点で進むカーソルなら、切断中のイベントを拾えて再送も起きない。
  - **jsonl の上限** (`CC_QUEUE_MAX_LINES`、既定 2000 → 1600 行に切り詰め): temp + rename で原子的に。**トリムしたらログに出す** (黙って落とさない)。
  - `docs/setup-cc-remote.md` を新設。**CC 自身が読んで実行する**形の手順書で、人間は raw URL を 1 本渡すだけ。★ **パスワードは人間が書く** (`~/.tealus/cc-auth.json` を `chmod 600`)。
- **サーバが計画的な停止を自分で予告する (`__bye`)** ([#365](https://github.com/gamasenninn/tealus/issues/365)): SIGINT / SIGTERM で `{"__bye":{"reason":"shutdown","expect_back_ms":30000}}` を全購読者へ流してから閉じる (`server.close()` より**先**)。当初は「再起動の前に人が宣言する」運用ルールで解こうとしたが、**バグ修正のたび × ブリッジ N 台**では忘れるようになり、忘れた時点で本当に必要な場面でも使われなくなる (実際、宣言運用を決めた当日の午前に決めた本人が破っている)。**情報を持っているサーバが自分で言えば、覚えておく必要が消える。**
  - ★ **予告できるものは予告し、予告できないものは異常として残る** — この予告が出るのは graceful shutdown のときだけで、クラッシュや `kill -9` では出ない。狙って作るのではなく仕組みからそうなる。
  - 消費側は**時間の猶予窓**として持つ (回数ではなく)。「次の 1 回の切断だけ静音」だと**再接続失敗で結局起こされる**うえ、切断が来なかったときフラグが残る。期限なら自動失効するので**失効処理を書く必要すら無い**。
  - `expect_back_ms` を**サーバが渡す**のは、クライアントが決め打つと #361 で解いた問題 (サーバの都合をクライアントがハードコードする) を再導入するため。クライアント側は上限 300 秒で clamp する。
- **`/cc-queue/pending` がサーバの設定値を返す** ([#361](https://github.com/gamasenninn/tealus/issues/361)): 応答に `max_age_ms` / `heartbeat_ms` を追加。無いとクライアントは値をハードコードするしかなく、**サーバ側で `max_age` を変えた瞬間に全ての切断が「想定外」に落ちて通知の嵐になる**。既存フィールドは不変なので古いクライアントは無視するだけ。
- **リリース チェックリスト** (`docs/release-checklist.md`): 「**外部から見える数字**」(Node 要件 / MCP ツール数 / リアクション数 / TTS の声 / migration 番号) の差分を下流 (LP・tealus-docs・README) に流すところまでをリリースと定義。v0.7.0 でリアクション数が**3 か所で同時に古い**状態になっていたのが起点。★ 本版の作業で、実際に古い MCP ツール数 (11 / 15 → **18**) を捕まえた。

### Changed

- **★ Node 要件の宣言を実態に揃えた** (`0cf9684`): `engines` を 6 パッケージとも `>=24.0.0` へ。Dockerfile / CI と実行時要件は以前から 24。
- **予定どおりの切断で LLM を起こさない** ([#363](https://github.com/gamasenninn/tealus/issues/363)): 消費側の分類を stdout (通知 = 起こす) と stderr (記録のみ = 起こさない) に分けた。**Monitor は stdout の行だけを通知にする**という仕様を使っている。
  - 判定は `curl の終了コード = 0` **かつ** `MAX_AGE <= 経過秒 <= MAX_AGE + 5` の両方。窓を「9 割」のように広く取ると **330 秒幅**になり、サイクル末尾の予定外の再起動 (実測 2,978 秒) が黙って埋もれる。**間違えるなら狭い側** (余計な通知は安く、見逃しは高い)。
  - **障害継続中の間引き** (1・2・4・8… 回目のみ通知): 1 時間の停止で 450 回前後の起床が発生し、**「イベントが多すぎる監視は自動停止される」仕様で監視そのものが死ぬ** = 障害のときに限って監視が死ぬ、という形を踏む前に塞いだ。
  - **復帰通知** (`recovered after N attempts, Ms down`) と**1 日 1 回の生存行**。★ 生存行は死活監視ではない (「来るはずのものが来ない」と誰かが気づいて初めて機能する) ことを明記し、サーバ側の本当の死活監視は別途検討として [#364](https://github.com/gamasenninn/tealus/issues/364) に記録した (現状は人間が気づく経路が機能しているため未実装)。
- **client chat 中核のリファクタ (継続分)** ([#346](https://github.com/gamasenninn/tealus/issues/346)): `onChange` の順序依存分岐とメンション挿入を純関数化。
- **README のロードマップを薄く作り直した** (`d21e941` / `0a0bb0f`): 旧節は 69 行に育って腐り、「これから」の 10 項目中 9 項目が既に CLOSED、「現在 v0.2.4」が 5 リリース前という状態で、読者には**実態と正反対** (止まっているプロジェクト) に見えていた。バージョン番号・完了項目・日付入りの経過を**書かない**ルールにして再作成し、見直しをリリース手順に含めた。

### Fixed

- **★ cc-queue ストリームの認可が接続時のスナップショットのまま更新されない** ([#360](https://github.com/gamasenninn/tealus/issues/360)): `allowedRooms` は接続時に `/api/rooms` を引いた結果を保持し続け、JWT の検証も入口で 1 回だけ。**ルームから外された / ユーザーが無効化された / トークンが失効した、のどれも開いている接続を止めなかった**。実測で 2 時間 26 分無切断の接続が出て顕在化 (無切断を達成したことで初めて見えた穴)。`CC_STREAM_MAX_AGE_MS` (既定 55 分) でサーバ側から閉じ、クライアントの再接続ループに**再ログイン + 再認可**をさせる。取りこぼしは `since` が防ぐので消費側の変更は不要。既定を 55 分にしているのは nginx の `proxy_read_timeout 3600s` より短く取り、タイムアウト境界での不定な切れ方より**自分の意図した切断を先に来させる**ため。
- **停止時のログが書き出される前に `process.exit` していた** (`72b67f2`): winston の file transport は非同期で、`__bye` は正しく送信されているのに「停止を予告しました」の行だけがログから消えていた (受信側で自分の curl を張って直接受信し確定)。★ **動作は最初から正しく、欠けていたのは見え方だけ。**ログが無いことを根拠に「送信されていない」と判断していたら、存在しない不具合を追っていた。
- **ストリームの仕分けが除外方式で、エラー本文が受信ログを壊していた** (`f3cd127`): `grep -v '"__hb"'` だと proxy の 504 などが返す HTML が素通りし、`since` の計算が壊れて空になる → **再接続で切断中の mention を丸ごと取りこぼす** (切断を拾うための仕組みが切断のたびに壊れる)。`{"id"` で始まる行だけをログに入れる**許可方式**に変更。
- **消費側が改行で終わらない最終行を捨てていた** (`3792947`): `while IFS= read -r line` は末尾に改行が無い最終行を落とすため、**proxy のエラー本文が読めずに消えていた**。「エラー 0 件」という観測が「エラーが無い」ではなく「エラーを読めていない」を意味していた。`|| [ -n "$line" ]` で修正。あわせて curl の終了コードを拾うようにした (**エラー本文と終了コードは片方では足りず両方要る** — proxy のエラー本文が返っても curl 自体は 0 で終了する)。
- **テストが本番の agent-server ログを汚していた** (`f3cd127`): `ccSubscribers.test.mts` に logger のモックが無く、テストデータが本番ログに混入していた。

### ドキュメント

- `docs/05_実装ノート_不変条件と落とし穴.md` に cc-bridge の不変条件を追加: **2 つの出口を同時に有効化しない** / **cc-queue dir は単一 agent-server が所有** / **`allowedRooms` は必須** / **`since` は watermark ではない** / **認可はスナップショットなので接続に寿命が要る** / **仕分けは許可方式** / **Monitor は exit で監視ごと終わる**。
- 設計上の判断として **「片側しか持っていない情報は、持っている側から配る」**(運用ルールは情報を持たない側に「覚えておけ」と要求するので必ず腐る) と **「状態を回数でなく時間で持つと後始末が不要になる」** を記録。前者には**後付けの言語化でまだ予測に使えた実績が無い**という留保を明記した。
- 検証の限界として **「動作確認は『意図どおりか』しか見ない。『意図が正しいか』は別に問う」** と **「測れるものについて推論しない」** を記録。
- `docs/setup-cc-tealus-bridge.md`: skill の配置手順がディレクトリ形式でなく**存在しないファイル**を案内していたのを修正。MCP ツール数を 11 / 15 → **18** に更新 (版で変わるので版を添えた)。
