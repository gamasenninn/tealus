...
jest.mock('../../src/router/index.mts', () => ({
  route: jest.fn(),
}));
...
jest.mock('../../src/agents/lightV2.mts', () => {
...
jest.mock('../../src/media/messageAdapter.mts', () => ({
  extractPromptFromMessage: jest.fn((msg: { content?: string | null } | null | undefined) => msg?.content || ''),
}));
...
jest.mock('../../src/webhook/delegator.mts', () => {
  const actual = jest.requireActual('../../src/webhook/delegator.mts');
...
describe('Dispatcher', () => {
  ...
  describe('% delegation 結線 (#295)', () => {
    ...
    test('(e) 委譲先の非メンバーは委譲拒否、委譲元へ通知 (#282 権限チェック)', async () => {
      ...
      expect(handleDelegation).not.toHaveBeenCalled();
      expect(route).not.toHaveBeenCalled();
      const denial = botApi.pushMessage.mock.calls.find(([rid]: [string]) => rid === 'room1');
    ...
    test('(g) 複数 %room → handleMultiDelegation 呼出 (単室 handleDelegation でない)', async () => {
      ...
      expect(arg.originRoomId).toBe('room1');
      expect(arg.targets.map((t: { name: string }) => t.name)).toEqual(['朝礼', '終礼']);
    ...
    test('(h) 多室で一部が非メンバー → 除外注記、許可1室なら単室 handleDelegation', async () => {
      ...
      botApi.isRoomMember.mockImplementation((rid: string) => Promise.resolve(rid === 'r-asa'));
      ...
      // 除外注記が委譲元へ
      const skipNote = botApi.pushMessage.mock.calls.find(([, text]: [string, string]) => /権限がないため除外/.test(text));
