...
describe('inflightRooms (#292 SPIKE)', () => {
  ...
  test('null / undefined roomId は no-op', () => {
    expect(() => inflightRooms.add(null as unknown as string)).not.toThrow();
    expect(() => inflightRooms.release(undefined as unknown as string)).not.toThrow();
    expect(inflightRooms.isInflight(null as unknown as string)).toBe(false);
  });
});
