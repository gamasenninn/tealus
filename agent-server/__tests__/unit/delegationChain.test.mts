 ...
 */
import * as chain from '../../src/webhook/delegationChain.mts';

// startChain / extendChain は非 export の ChainResult 判別共用体 (discriminated union) を返す。
// テスト内では成否確定後に .envelope / .reason へ直接アクセスするため (= 元 JS と同じ書き味を保つ)、
// 判別済みとみなす境界キャストを 1 箇所に集約する。
type StartResult = ReturnType<typeof chain.startChain>;
function asOk<T extends { ok: boolean }>(r: T): Extract<T, { ok: true }> {
  return r as Extract<T, { ok: true }>;
}
function asErr<T extends { ok: boolean }>(r: T): Extract<T, { ok: false }> {
...
describe('delegationChain (#295 予算付き封筒 + 4 段ガード)', () => {
  ...
  describe('startChain (root 委譲)', () => {
    test('封筒を生成する: hop=0, visited=[origin,target], task 保持', () => {
      ...
      expect(asOk(r).envelope.hop).toBe(0);
      expect(asOk(r).envelope.visited).toEqual(['A', 'B']);
      expect(asOk(r).envelope.originRoomId).toBe('A');
      expect(asOk(r).envelope.targetRoomId).toBe('B');
      expect(asOk(r).envelope.task).toBe('集計して');
      expect(typeof asOk(r).envelope.chainId).toBe('string');
      expect(asOk(r).envelope.chainId.length).toBeGreaterThan(0);
      expect(typeof asOk(r).envelope.nodeId).toBe('string');
    });
    ...
    test('2 回呼ぶと別 chainId になる', () => {
      ...
      expect(asOk(a).envelope.chainId).not.toBe(asOk(b).envelope.chainId);
    });

    test('自室委譲 (origin === target) は cycle で弾く', () => {
      const r = chain.startChain({ originRoomId: 'A', targetRoomId: 'A', task: 't' });
      expect(r.ok).toBe(false);
      expect(asErr(r).reason).toBe('cycle');
    });
  });

  describe('extendChain (子委譲)', () => {
    test('正常: hop が増え visited に target が追加され chainId は継承', () => {
      const root = asOk(chain.startChain({ originRoomId: 'A', targetRoomId: 'B', task: 't' })).envelope;
      const r = chain.extendChain(root, { targetRoomId: 'C', task: 't2' });
      expect(r.ok).toBe(true);
      expect(asOk(r).envelope.hop).toBe(1);
      expect(asOk(r).envelope.visited).toEqual(['A', 'B', 'C']);
      expect(asOk(r).envelope.chainId).toBe(root.chainId);
      expect(asOk(r).envelope.nodeId).not.toBe(root.nodeId);
    });

    test('cycle: visited に既出の room へは委譲拒否', () => {
      const root = asOk(chain.startChain({ originRoomId: 'A', targetRoomId: 'B', task: 't' })).envelope;
      expect(asErr(chain.extendChain(root, { targetRoomId: 'A', task: 't' })).reason).toBe('cycle');
      expect(asErr(chain.extendChain(root, { targetRoomId: 'B', task: 't' })).reason).toBe('cycle');
    });

    test('max_depth: hop が maxDepth を超えたら拒否', () => {
      chain.configure({ maxDepth: 2, maxChainBudget: 100, maxFanoutPerTurn: 100 });
      const root = asOk(chain.startChain({ originRoomId: 'A', targetRoomId: 'B', task: 't' })).envelope; // hop0
      const c1 = chain.extendChain(root, { targetRoomId: 'C', task: 't' }); // hop1
      expect(c1.ok).toBe(true);
      const c2 = chain.extendChain(asOk(c1).envelope, { targetRoomId: 'D', task: 't' }); // hop2
      expect(c2.ok).toBe(true);
      const c3 = chain.extendChain(asOk(c2).envelope, { targetRoomId: 'E', task: 't' }); // hop3 > 2
      expect(c3.ok).toBe(false);
      expect(asErr(c3).reason).toBe('max_depth');
    });
    ...
    test('fanout_exceeded: 同一 parent から maxFanoutPerTurn を超える分岐を拒否', () => {
      ...
      expect(asErr(third).reason).toBe('fanout_exceeded');
    });
    ...
    test('budget_exhausted: chain 全体の委譲総数が maxChainBudget に達したら拒否 (木の形に依らない)', () => {
      ...
      const root = asOk(chain.startChain({ originRoomId: 'A', targetRoomId: 'B', task: 't' })).envelope; // spent=1
      ...
      expect(asErr(over).reason).toBe('budget_exhausted');
    });
    ...
    test('予算は枝をまたいで共有される (扇状増殖でも総量で止まる)', () => {
      ...
      const d = chain.extendChain(asOk(c).envelope, { targetRoomId: 'D', task: 't' }); // spent=3
      expect(d.ok).toBe(true);
      // 別枝 (root の子) でも同じ予算を共有 → もう取れない
      const e = chain.extendChain(root, { targetRoomId: 'E', task: 't' });
      expect(e.ok).toBe(false);
      expect(asErr(e).reason).toBe('budget_exhausted');
    });
  });

  test('未知 chainId の封筒 (偽造 / reset 後の孤児) への extend は拒否', () => {
    const fake: Extract<StartResult, { ok: true }>['envelope'] = {
      chainId: 'nonexistent',
      nodeId: 'x',
      hop: 0,
      visited: ['A', 'B'],
    ...
    expect(r.ok).toBe(false);
    expect(asErr(r).reason).toBe('unknown_chain');
  });
