...
// jest.resetModules() 後に fresh module state (module-scope `cached` / mock call history) を
// 得るため require() で動的に再取得する。static import では resetModules の効果が出ない
// (import binding は 1 回だけ解決される) ため、light.mts の getCodex パターンと同様に
// createRequire で明示的に require を用意する。
...
type LoaderModule = typeof import('../../src/agents/lightBackendLoader.mts');
...
describe('loadLightBackend (#292)', () => {
  ...
  beforeEach(() => {
    ...
    logger = (require('../../src/lib/logger.mts').logger) as MockLogger;
    loader = require('../../src/agents/lightBackendLoader.mts') as LoaderModule;
    loader.resetForTest();
  });
