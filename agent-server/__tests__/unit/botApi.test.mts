...
describe('botApi.request status handling (#303)', () => {
  ...
  it('(b) 非2xx は status/body を持つ Error を throw し logger.error する', async () => {
    ...
    const err = await botApi.pushStatus('room1', 'idle').catch((e: Error & { status?: number; body?: string }) => e);
  ...
  it('(d) 401 retry 後も失敗なら throw (fetch=4回)', async () => {
    ...
    const err = await botApi.pushStatus('room1', 'idle').catch((e: Error & { status?: number }) => e);
    expect(err).toBeInstanceOf(Error);
