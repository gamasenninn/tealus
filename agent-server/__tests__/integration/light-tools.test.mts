...
// createTools が返すツールの実行時 shape (production の Tool 型 (invoke ベース) とは別物。
// zod/@openai/agents をモックしているため execute ベースの簡易 shape で受ける — 意図的な cast)
...
describe('Light Agent カスタムツール統合テスト', () => {
  let tools: MockToolResult[];

  beforeEach(() => {
    // tool モックを設定
    jest.mock('@openai/agents', () => ({
      tool: jest.fn((opts: { name: string; execute: MockToolResult['execute'] }) => ({ name: opts.name, execute: opts.execute, _type: 'tool' })),
    }));
    jest.mock('zod', () => ({
    ...
    // ESM では import が巻き上げられ評価されるため、上記 per-test jest.mock (@openai/agents / zod)
    // より前に static import 経由で lightTools.mts が読み込まれてしまう (実 zod/tool で組み立て
    // られ、mock が effective にならない)。require で mock 登録後の遅延読込にする。
    const { createTools } = require('../../src/agents/lightTools.mts') as { createTools: (workspacePath: string, roomId: string | null) => MockToolResult[] };
    tools = createTools(tmpDir, 'room1');
  });

  function findTool(name: string): MockToolResult | undefined {
  ...
  test('5. generate_image 成功 → pushImage 呼ばれる', async () => {
    const botApi = require('../../src/lib/botApi.mts') as { pushImage: jest.Mock };
