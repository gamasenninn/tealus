...
export interface PolysemeEntry {
  filename: string;
  term: string;
  content: string;
}

export class OrganonReloader {
  repoPath: string;
  cached: PolysemeEntry[] | null;

  constructor({ repoPath = DEFAULT_ORGANON_PATH }: { repoPath?: string } = {}) {
   ...
   */
  loadPolysemeEntries(): PolysemeEntry[] {
   ...
   * に robust。
   *
   * @param entries - load 済 entries (= 省略時は cached or 新規 load)
   * @returns prompt block (= 空 entries 時は空文字)
   */
  formatForPrompt(entries?: PolysemeEntry[]): string {
