/**
 * #439 — エージェントに渡す知識の配線を、宣言表で 1 か所に置く。
 *
 * ★ **結線済み。** `partsFor` を呼んでいるのは 4 経路:
 *   `webhook/dispatcher.mts` (Deep / 委譲 / 統合) / `agents/light.mts` (v1) /
 *   `agents/lightV2.mts` / `routes/voiceChat.mts` (会話モード)。
 *
 * ★★★★ **2026-09-18 訂正**: ここには Step 1 当時の「★ まだ誰も呼ばない。この段階では
 *   挙動を 1 byte も変えない」が残っていた。★ Step 2/3 で結線した後も直していなかったため、
 *   **この file を読んだ側が「表は使われていない」と誤読した** (実際に 1 件起きた)。
 *   ★★ すぐ下の「表は契約であって現状ではない」という警告の **隣に、古い現状説明が同居**
 *   していたのが質の悪いところ。★★★ **注意書き自体も古くなる。**
 *
 * ## なぜ要るか
 *
 * 配線が 2 流派に割れている:
