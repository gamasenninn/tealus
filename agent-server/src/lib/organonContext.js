...
function loadOrganonPolysemeForPrompt(options = {}) {
  ...
  return [
    ...
    '',
    'DB 検索 query 時、以下の業務語彙 ↔ DB column 値 mapping を参照して SQL を生成してください。「納品」「売上」等の業務語を SQL の column 値 / WHERE 条件に展開する時、organon entry の sql_mapping field を ground truth として使ってください:',
    '',
    ...blocks,
    '',
  ].join('\n');
