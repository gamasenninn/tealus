 ...
 *
 *   ★★ 既定 55 分の由来は**誤った前提**だった (2026-08-22 に NAS 上で `nginx -T` を実測、docs/05)。
 *   ここには「nginx の `proxy_read_timeout` 3600s より短く取るため」と書いてあったが、
 *   `app.tealus.dev` の server ブロックは **60s** (connect / read / send とも)。3600s は別ブロックの値。
 *   → ★ **60s の無通信切断を抑えているのは max_age ではなく 15 秒ごとの heartbeat** (4 倍の余裕)。
 *     55 分という値自体は害が無いので変えていないが、**伸縮を検討するときは 60s と heartbeat の
 *     関係で考えること** (「3600s より短いから安全」は成り立たない)。
 *   ★★★ この訂正は 2026-08-22 に docs/05 の本文へ入ったが、**このコメントは 20 日間古いままで、
 *     2026-09-11 の調査ブリーフに 3600s を再感染させた**。本文とコメントは同時に直す。
 */
