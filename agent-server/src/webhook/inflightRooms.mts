 ...
 * dispatcher 処理開始時に呼ぶ。
 */
function add(roomId: string): void {
  if (!roomId) return;
 ...
 * @param delayMs テスト用に上書き可能
 */
function release(roomId: string, delayMs: number = RELEASE_DELAY_MS): void {
  if (!roomId) return;
 ...
 */
function isInflight(roomId: string): boolean {
...
export { add, release, isInflight, _reset, RELEASE_DELAY_MS };
