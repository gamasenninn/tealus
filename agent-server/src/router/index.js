 ...
 * - DEEP_AVAILABLE=true: そのまま返す
 * - DEEP_AVAILABLE=false かつ tier='deep' の場合:
 *   - isExplicitDeep (/deep 指定) → 'unavailable' に変換 (dispatcher が説明メッセージを返す)
 *   - それ以外 (キーワード/LLM) → 'light' に silent fallback
 */
function applyDeepAvailability(result, isExplicitDeep) {
  if (config.DEEP_AVAILABLE) return result;
  if (result.tier !== 'deep') return result;

  if (isExplicitDeep) {
    return { ...result, tier: 'unavailable' };
  }
  logger.debug('Router: deep fallback to light (claude CLI unavailable)');
