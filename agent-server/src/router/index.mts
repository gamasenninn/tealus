...
// 挨拶パターン
const GREETING_PATTERNS: Array<{ pattern: RegExp; response: string }> = [
 ...
 * 第1段: ルールベース分類
 */
function classifyByRules(content: string): RouteResult | null {
 ...
 * 第2段: LLM による分類
 */
async function classifyByLLM(content: string): Promise<RouteResult> {
  try {
...
1単語だけ返してください。`,
    ...
    logger.error(`Router LLM error: ${err instanceof Error ? err.message : String(err)}`);
    // フォールバック: Lightに振る
    return { tier: 'light', prompt: content };
  }
}
 ...
 *
 * 可用性なし時:
 *   - isExplicitDeep (/deep 指定) → 'unavailable' に変換 (dispatcherが説明メッセージを返す、provider field 付き)
 *   - それ以外 (キーワード/LLM) → 'light' に silent fallback
 */
function applyDeepAvailability(result: RouteResult, isExplicitDeep: boolean): RouteResult {
 ...
 * 統合ルーティング: 第1段 → 第2段
 */
async function route(content: string): Promise<RouteResult> {
