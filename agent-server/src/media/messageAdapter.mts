 ...
 */
import type { WebhookMessage } from '../types.mts';

/**
 * Webhookペイロードのメッセージからエージェント用プロンプトを抽出
 * @param message - Webhookのmessageオブジェクト
 * @returns エージェントに渡すプロンプト
 */
export function extractPromptFromMessage(message: WebhookMessage | null | undefined): string {
  if (!message) return '';

  const type = message.type || 'text';
  const content = message.content || '';
  // 元実装から計算のみ・未使用の変数（挙動保持のため残置）
