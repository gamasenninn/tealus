...
export interface ProcessLightArgs {
  /** Tealus room UUID (必須) */
  roomId: string;
  /** 完成済 user prompt (= room ID embed + reply_to hint 含む) */
  prompt: string;
  /** room 専用 workspace の絶対 path */
  workspacePath?: string;
  /** agent registration UUID (status 更新等で必要なら) */
  agentId?: string;
  /** session UUID (context 引継ぎが必要なら) */
  sessionId?: string;
  /** MCPServerStdio instance 配列 (V1 系のみ使用、自作は ignore 可) */
  mcpServers?: unknown[];
}

export async function processLight({ roomId, prompt, workspacePath, agentId, sessionId, mcpServers }: ProcessLightArgs): Promise<void> {
  try {
    ...
    logger.error(`[LightCustomExample] error: ${message}`);
    // error 時も pushMessage で user に伝える (= throw しない)
    await botApi.pushMessage(roomId, `エラー: ${message}`).catch(() => {});
    await botApi.pushStatus(roomId, 'idle').catch(() => {});
  }
}
