 ...
 * claude -p の引数を構築
 */
export function buildClaudeArgs({ workspacePath, sessionId }: { workspacePath: string; sessionId?: string }): string[] {
...
export interface ProcessDeepArgs {
  roomId: string;
  prompt: string;
 ...
 */
export async function processDeep({ roomId, prompt, workspacePath, agentId, sessionId }: ProcessDeepArgs): Promise<void> {
  return new Promise((resolve) => {
    ...
    proc.on('close', async (code) => {
      ...
      if (timedOut) {
        ...
        await botApi.pushMessage(roomId, `⚠ タイムアウトしました（${Math.round(config.DEEP_TIMEOUT / 1000)}秒超過）。タスクが複雑すぎる可能性があります。`)
          .catch((err) => logger.error(`[Deep] timeout notice NOT delivered to room ${roomId}: ${err instanceof Error ? err.message : String(err)}`));
        resolve();
        return;
      }
      ...
      if (code !== 0 && !stdout) {
        ...
        await botApi.pushMessage(roomId, `❌ エラーが発生しました: ${stderr.slice(0, 200) || 'Unknown error'}`)
          .catch((err) => logger.error(`[Deep] error notice NOT delivered to room ${roomId}: ${err instanceof Error ? err.message : String(err)}`));
        resolve();
        return;
      }
      ...
      if (response) {
        ...
        try {
          ...
          const message = err instanceof Error ? err.message : String(err);
          logger.error(`[Deep] response NOT delivered to room ${roomId} (${response.length} chars): ${message}`);
        }
      }
    ...
    proc.on('error', async (err) => {
      ...
      await botApi.pushMessage(roomId, `❌ Deep Agent の起動に失敗しました: ${err.message}`)
        .catch((e) => logger.error(`[Deep] spawn-error notice NOT delivered to room ${roomId}: ${e instanceof Error ? e.message : String(e)}`));
      resolve();
    });
  });
}
