 ...
 *
 * 現 Deep agent (= `claude -p` spawn、deep.mts) の Codex CLI 版 mirror。
 ...
 *       Light v2 (#258、lightV2.mts) で確立済の Codex SDK auth pattern を CLI mode で再現
 */
...
export type McpServersConfig = Record<string, McpServerDef>;

/** cancel / registry 連携用に ChildProcess へ付与する独自プロパティ */
export type TealusChildProcess = ChildProcess & {
  _tealusCancelled?: boolean;
  _tealusWorkspacePath?: string;
  _tealusSafetyNetFired?: boolean;
  _tealusTimer?: NodeJS.Timeout;
};

/** codex exec --json の JSONL event (消費するフィールドのみ) */
 ...
 */
export function tomlEscape(s: unknown): string {
 ...
 *
 * 入力 = deep.mts の createDeepMcpConfig 同 form:
 *   {
 ...
 *   TEALUS_API_URL = "..."
 */
export function serializeMcpServersToToml(mcp_servers: McpServersConfig): string {
  const lines: string[] = [];
 ...
 * @returns 書き戻したら true
 */
export function writeBackCodexAuth(codexHomePath: string, codexHomeSrcDir: string): boolean {
  try {
    ...
    logger.warn(`[deepCodex] auth.json writeback failed: ${err instanceof Error ? err.message : String(err)}`);
    return false;
  }
}
 ...
 */
export function buildCodexExecArgs({ workspacePath, model }: { workspacePath: string; model: string }): string[] {
  return [
 ...
 */
export function buildCodexExecEnv({ codexHomePath, openaiApiKey, useSubscription, baseEnv }: {
...
export class JsonlLineBuffer {
  ...
  append(chunk: string): CodexJsonlEvent[] {
    this.buffer += chunk;
    const lines = this.buffer.split('\n');
    this.buffer = lines.pop() || ''; // 最後は incomplete or 空
    const results: CodexJsonlEvent[] = [];
 ...
 *
 * 「最後の非空 agent_message を採用」が正しい (= lightV2.mts 確立済 pattern)。
 */
export function isAgentMessageEvent(event: CodexJsonlEvent | null | undefined): boolean {
...
export function extractAgentMessageText(event: CodexJsonlEvent | null | undefined): string {
 ...
 */
export function isToolCallSendMessageToOwnRoom(event: CodexJsonlEvent | null | undefined, roomId: string): boolean {
...
export interface ProcessDeepCodexParams {
  roomId: string;
  prompt: string;
  workspacePath: string;
  agentId?: string | null;
...
/**
 * Deep Codex Agent でメッセージを処理 (= deep.mts processDeep の Codex 版 mirror)
 *
 * 返り値: 最終 agent message 本文 (無ければ null。timeout / cancel / safety net 経路は
 * 旧実装同様 undefined で resolve — 呼び出し側は falsy 判定のみに依存すること)
 */
export async function processDeepCodex({ roomId, prompt, workspacePath, agentId, sessionId, mcpServers, suppressAutoPost = false }: ProcessDeepCodexParams): Promise<string | null | undefined> {
  return new Promise<string | null | undefined>((resolve) => {
    // #295: 委譲 (runAgent) 経由は suppressAutoPost=true。自室へ投稿/エラー通知せず本文を
    //       resolve で返し、デリゲーターが委譲元へ機械配送する (委譲先に残さない)。
    const postTarget = (txt: string) => (suppressAutoPost ? Promise.resolve() : botApi.pushMessage(roomId, txt));
    ...
    const timer = setTimeout(async () => {
      ...
      // safety net (= deep.mts と同型 10s 後 force resolve)
      setTimeout(() => {
    ...
    proc.on('close', async (code) => {
      ...
      if (lastAgentMessage) {
        ...
        if (suppressAutoPost) {
          ...
          const content: string = lastAgentMessage;
          ...
          try {
            ...
            logger.error(`[DeepCodex] response NOT delivered to room ${roomId} (${content.length} chars): ${err instanceof Error ? err.message : String(err)}`);
          }
        }
      } else {
